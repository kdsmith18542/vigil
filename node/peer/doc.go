...
caller to specify things such as the user agent name and version, the Vigil
network to use, which services it supports, and callbacks to invoke when Vigil
...
In order to do anything useful with a peer, it is necessary to react to Vigil
...
For convenience, a callback hook for all of the currently supported Vigil
