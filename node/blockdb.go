...
func removeRegressionDB(dbPath string) error {
	...
	if err == nil {
		vgldLog.Infof("Removing regression test database from '%s'", dbPath)
...
func warnMultipleDBs() {
	...
	if len(duplicateDbPaths) > 0 {
		...
		vgldLog.Warnf("WARNING: There are multiple block chain databases "+
...
func loadBlockDB(params *chaincfg.Params) (database.DB, error) {
	...
	if cfg.DbType == "memdb" {
		vgldLog.Infof("Creating block database in memory.")
	...
	vgldLog.Infof("Loading block database from '%s'", dbPath)
	...
	if err := blockchain.CheckDBTooOldToUpgrade(db); err != nil {
		...
		vgldLog.Infof("Removing database from '%s': %v", dbPath, err)
	...
	vgldLog.Info("Block database loaded")
	return db, nil
}
...
func dumpBlockChain(params *chaincfg.Params, b *blockchain.BlockChain) error {
	vgldLog.Infof("Writing the blockchain to flat file %q.  This might take a "+
		"while...", cfg.DumpBlockchain)

	progressLogger := progresslog.New("Wrote", vgldLog)
