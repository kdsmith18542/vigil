...
// BenchmarkvgldRead benchmarks reading random values via the global Read method
// with various size reads.
func BenchmarkvgldRead(b *testing.B) {
...
// BenchmarkvgldReadPRNG benchmarks reading random values via a local PRNG
// instance with various size reads.
func BenchmarkvgldReadPRNG(b *testing.B) {
	benches := makeReadBenches()
	for benchIdx := range benches {
		bench := benches[benchIdx]
		b.Run(bench.name, func(b *testing.B) {
