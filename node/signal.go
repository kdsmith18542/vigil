...
func shutdownListener() context.Context {
	...
	go func() {
		...
		case sig := <-interruptChannel:
			vgldLog.Infof("Received signal (%s).  Shutting down...", sig)

		case <-shutdownRequestChannel:
			vgldLog.Infof("Shutdown requested.  Shutting down...")
		}
		cancel()
		...
		for {
			select {
			case sig := <-interruptChannel:
				vgldLog.Infof("Received signal (%s).  Already "+
					"shutting down...", sig)

			case <-shutdownRequestChannel:
				vgldLog.Info("Shutdown requested.  Already " +
					"shutting down...")
			}
		}
	}()
