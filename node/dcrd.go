...
// winServiceMain is only invoked on Windows.  It detects when vgld is running
...
// vgldMain is the real main function for vgld.  It is necessary to work around
...
func vgldMain() error {
	...
	defer vgldLog.Info("Shutdown complete")

	// Show version and home dir at startup.
	vgldLog.Infof("Version %s (Go version %s %s/%s)", version.String(),
		runtime.Version(), runtime.GOOS, runtime.GOARCH)
	vgldLog.Infof("Home dir: %s", cfg.HomeDir)
	if cfg.NoFileLogging {
		vgldLog.Info("File logging disabled")
	}
	...
	if goMemLimit < softMemLimit {
		vgldLog.Infof("Ignoring GOMEMLIMIT=%q since it is too low and would "+
	...
	vgldLog.Infof("Soft memory limit: %s", humanizeBytes(softMemLimit))
	...
	if cfg.Profile != "" {
		...
		if err := profiler.Start(cfg.Profile, allowNonLoopback); err != nil {
			vgldLog.Warnf("unable to start profile server: %v", err)
			return err
		}
	}
	...
	if cfg.CPUProfile != "" {
		...
		if err != nil {
			vgldLog.Errorf("Unable to create cpu profile: %v", err)
			return err
		}
	...
	if cfg.MemProfile != "" {
		...
		if err != nil {
			vgldLog.Errorf("Unable to create mem profile: %v", err)
			return err
		}
		defer f.Close()
	...
	if err != nil {
		vgldLog.Errorf("%v", err)
		return err
	}
	defer func() {
		// Ensure the database is sync'd and closed on shutdown.
		lifetimeNotifier.notifyShutdownEvent(lifetimeEventDBOpen)
		vgldLog.Infof("Gracefully shutting down the block database...")
		db.Close()
	}()
	...
	defer func() {
		// Ensure the database is sync'd and closed on shutdown.
		vgldLog.Infof("Gracefully shutting down the UTXO database...")
		utxoDb.Close()
	}()
	...
	if err != nil {
		vgldLog.Errorf("Unable to start server: %v", err)
		return err
	}
