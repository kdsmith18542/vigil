...
func main() {
	...
	// Determine vgld app data directory based on environment variable.
	VigilData := os.Getenv("Vigil_DATA")
	vgldAppData := filepath.Join(VigilData, ".vgld")
	...
	switch arg0 {
	case "vgld":
		if !convertsToFalse(os.Getenv("vgld_NO_FILE_LOGGING")) {
			args = append(args, "--nofilelogging")
		}
		args = append(args, fmt.Sprintf("--appdata=%s", vgldAppData))
		args = append(args, "--rpclisten=")

	case "vglctl":
		// Determine vglctl app data directory based on environment variable.
		rpcCert := filepath.Join(vgldAppData, "rpc.cert")
		vglctlAppData := filepath.Join(VigilData, ".vglctl")
		vglctlConfig := filepath.Join(vglctlAppData, "vglctl.conf")
		...
		args = prepend(args, fmt.Sprintf("--configfile=%s", vglctlConfig))

		// Change the home directory to match the data path since vglctl
		// relies in it to discover the vgld config file in order to extract
		// the rpc credentials.
		if !fileExists(filepath.Join(vglctlAppData, "vglctl.conf")) {
			os.Setenv("HOME", VigilData)
		}
	}
