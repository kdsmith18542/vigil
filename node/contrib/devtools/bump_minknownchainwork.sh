...
# Ensure git, vglctl, and jq are available and this is running in a git repo.
if ! git status &> /dev/null; then
  echo "git repository not be found"
  exit 1
fi
if ! command -v vglctl &> /dev/null; then
  echo "vglctl could not be found"
  exit 1
fi
...
vglctl="vglctl"
vglctlt="vglctl --testnet"
MAINNET_BLOCK_HEIGHT=$($vglctl getblockcount)
MAINNET_BLOCK_HASH=$($vglctl getblockhash $MAINNET_BLOCK_HEIGHT)
MAINNET_CHAIN_WORK=$($vglctl getblockheader $MAINNET_BLOCK_HASH | jq -r .chainwork)
TESTNET_BLOCK_HEIGHT=$($vglctlt getblockcount)
TESTNET_BLOCK_HASH=$($vglctlt getblockhash $TESTNET_BLOCK_HEIGHT)
TESTNET_CHAIN_WORK=$($vglctlt getblockheader $TESTNET_BLOCK_HASH | jq -r .chainwork)
...
\`\`\`
\$ vglctl getblockhash ${MAINNET_BLOCK_HEIGHT} | vglctl getblockheader - | jq -r .chainwork
${MAINNET_CHAIN_WORK}
\$ vglctl --testnet getblockhash ${TESTNET_BLOCK_HEIGHT} | vglctl --testnet getblockheader - | jq -r .chainwork
${TESTNET_CHAIN_WORK}
\`\`\`
"
