...
const (
	// svcName is the name of vgld service.
	svcName = "vgldsvc"
	...
	svcDisplayName = "vgld Service"

	// svcDesc is the description of the service.
	svcDesc = "Downloads and stays synchronized with the Vigil block " +
...
// logServiceStartOfDay logs information about vgld when the main server has
...
// vgldService houses the main service handler which handles all service
// updates and launching vgldMain.
type vgldService struct{}
...
// long-running vgldMain (which is the real meat of vgld), handles service
// change requests, and notifies the service control manager of changes.
func (s *vgldService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start vgldMain in a separate goroutine so the service can start
...
// installService attempts to install the vgld service.  Typically this should
...
// removeService attempts to uninstall the vgld service.  Typically this should
...
// startService attempts to start the vgld service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &vgldService{})
	if err != nil {
