...
// processes to communicate with and manage the lifetime of a vgld child
...
// message, shutdown begins.  This prevents vgld from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			vgldLog.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child vgld process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	vgldLog.Errorf("Failed to write to pipe: %v", err)
}
...
// boundP2PListenAddrEvent are IPC events emitted by vgld with the bound local
...
// boundRPCListenAddrEvent are IPC events emitted by vgld with the bound local
