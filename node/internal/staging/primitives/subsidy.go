...
//
// Although changes to the subsidy are not expected, in light of VGLP0010, they
...
// associated VGLP.  While not the only possible approach, a good approach is
...
//
//	Prior to VGLP0010: PoW: 6, PoS: 3, Treasury: 1 => 6+3+1 = 10
//	   After VGLP0010: PoW: 1, PoS: 8, Treasury: 1 => 1+8+1 = 10
//
// Therefore, the subsidy split percentages are:
//
//	Prior to VGLP0010: PoW: 6/10 = 60%, PoS: 3/10 = 30%, Treasury: 1/10 = 10%
//	   After VGLP0010: PoW: 1/10 = 10%, PoS: 8/10 = 80%, Treasury: 1/10 = 10%
...
// Vigil launch or the modified subsidy split defined in VGLP0010 according to
// the provided flag.
//
...
//
// This function is safe for concurrent access.
func (c *SubsidyCache) CalcWorkSubsidy(height int64, voters uint16, useVGLP0010 bool) int64 {
	if !useVGLP0010 {
		// The work subsidy proportion prior to VGLP0010 is 60%.  Thus it is 6
		// since 6/10 = 60%.
		const workSubsidyProportion = 6
		return c.calcWorkSubsidy(height, voters, workSubsidyProportion)
	}

	// The work subsidy proportion defined in VGLP0010 is 10%.  Thus it is 1
	// since 1/10 = 10%.
...
// CalcStakeVoteSubsidy returns the subsidy for a single stake vote for a block
// using either the original subsidy split that was in effect at Vigil launch
// or the modified subsidy split defined in VGLP0010 according to the provided
// flag.
//
...
//
// This function is safe for concurrent access.
func (c *SubsidyCache) CalcStakeVoteSubsidy(height int64, useVGLP0010 bool) int64 {
	if !useVGLP0010 {
		// The vote subsidy proportion prior to VGLP0010 is 30%.  Thus it is 3
		// since 3/10 = 30%.
		const voteSubsidyProportion = 3
		return c.calcStakeVoteSubsidy(height, voteSubsidyProportion)
	}

	// The stake vote subsidy proportion defined in VGLP0010 is 80%.  Thus it is
	// 8 since 8/10 = 80%.
...
//
// This function is safe for concurrent access.
func (c *SubsidyCache) CalcTreasurySubsidy(height int64, voters uint16, useVGLP0006 bool) int64 {
	...
	// proportion.
	//
	// The treasury proportion is 10% both prior to VGLP0010 and after it.  Thus,
	...
	// prior to the point voting begins or when VGLP0006 is active.
	if height < stakeValidationHeight || useVGLP0006 {
		return subsidy
	}
