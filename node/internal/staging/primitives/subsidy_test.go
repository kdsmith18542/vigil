...
func TestSubsidyCacheCalcs(t *testing.T) {
	...
	tests := []struct {
		...
		wantWork        int64  // expected pow subsidy w/o VGLP0010
		wantWorkVGLP0010 int64  // expected pow subsidy with VGLP0010
		wantVote        int64  // expected single vote subsidy
		wantVoteVGLP0010 int64  // expected single vote subsidy w/o VGLP0010
		wantTrsy        int64  // expected treasury subsidy w/o VGLP0006
		wantTrsyVGLP0006 int64  // expected treasury subsidy with VGLP0006
	}{{
		...
		wantWorkVGLP0010: 168000000000000,
		wantVote:        0,
		wantVoteVGLP0010: 0,
		wantTrsy:        0,
		wantTrsyVGLP0006: 0,
		...
		wantFull:        3119582664,
		wantWork:        1871749598,
		wantWorkVGLP0010: 311958266,
		wantVote:        0,
		wantVoteVGLP0010: 0,
		wantTrsy:        311958266,
		wantTrsyVGLP0006: 311958266,
	}, {
		...
		wantFull:        3119582664,
		wantWork:        1497399678,
		wantWorkVGLP0010: 249566612,
		wantVote:        187174959,
		wantVoteVGLP0010: 499133226,
		wantTrsy:        249566612,
		wantTrsyVGLP0006: 311958266, // No reduction
	}, {
		...
		wantFull:        3088695706,
		wantWork:        1853217423,
		wantWorkVGLP0010: 308869570,
		wantVote:        185321742,
		wantVoteVGLP0010: 494191312,
		wantTrsy:        308869570,
		wantTrsyVGLP0006: 308869570,
	}, {
		...
		wantFull:        3088695706,
		wantWork:        1482573938,
		wantWorkVGLP0010: 247095656,
		wantVote:        185321742,
		wantVoteVGLP0010: 494191312,
		wantTrsy:        247095656,
		wantTrsyVGLP0006: 308869570, // No reduction
	}, {
		...
		wantFull:        3058114560,
		wantWork:        1834868736,
		wantWorkVGLP0010: 305811456,
		wantVote:        183486873,
		wantVoteVGLP0010: 489298329,
		wantTrsy:        305811456,
		wantTrsyVGLP0006: 305811456,
	}, {
		...
		wantFull:        1896827356,
		wantWork:        1138096413,
		wantWorkVGLP0010: 189682735,
		wantVote:        113809641,
		wantVoteVGLP0010: 303492376,
		wantTrsy:        189682735,
		wantTrsyVGLP0006: 189682735,
	}, {
		...
		wantFull:        1896827356,
		wantWork:        682857847,
		wantWorkVGLP0010: 113809641,
		wantVote:        113809641,
		wantVoteVGLP0010: 303492376,
		wantTrsy:        113809641,
		wantTrsyVGLP0006: 189682735, // No reduction
	}, {
		// First zero vote subsidy without VGLP0010.
		...
		// First zero work subsidy with VGLP0010
		...
		// First zero vote subsidy with VGLP0010.
		...
		// First zero work subsidy without VGLP0010.
	...
	for _, test := range tests {
		...
		if gotWork != test.wantWorkVGLP0010 {
			t.Errorf("%s: unexpected work subsidy result (VGLP0010) -- got %d, "+
				"want %d", test.name, gotWork, test.wantWorkVGLP0010)
			continue
		}
		...
		if gotVote != test.wantVoteVGLP0010 {
			t.Errorf("%s: unexpected vote subsidy result (VGLP0010) -- got %d, "+
				"want %d", test.name, gotVote, test.wantVoteVGLP0010)
			continue
		}
		...
		if gotTrsy != test.wantTrsyVGLP0006 {
			t.Errorf("%s: unexpected treasury subsidy result (VGLP0006) -- got "+
				"%d, want %d", test.name, gotTrsy, test.wantTrsyVGLP0006)
			continue
		}
	}
}
...
func TestTotalSubsidy(t *testing.T) {
	...
	checkTotalSubsidy := func(useVGLP0006 bool) {
		t.Helper()
		...
		subsidySum := func(height int64) int64 {
			const useVGLP0010 = false
			work := cache.CalcWorkSubsidy(height, votesPerBlock, useVGLP0010)
			vote := cache.CalcStakeVoteSubsidy(height, useVGLP0010)
			vote *= int64(votesPerBlock)
			trsy := cache.CalcTreasurySubsidy(height, votesPerBlock, useVGLP0006)
		...
		if totalSubsidy != expectedTotalSubsidy {
			t.Fatalf("mismatched total subsidy (treasury flag: %v) -- got %d, "+
				"want %d", useVGLP0006, totalSubsidy, expectedTotalSubsidy)
		}
	}
...
func TestTotalSubsidyVGLP0010(t *testing.T) {
	...
	subsidySum := func(height int64, useVGLP0010 bool) int64 {
		const useVGLP0006 = false
		work := cache.CalcWorkSubsidy(height, votesPerBlock, useVGLP0010)
		vote := cache.CalcStakeVoteSubsidy(height, useVGLP0010) *
			int64(votesPerBlock)
		treasury := cache.CalcTreasurySubsidy(height, votesPerBlock, useVGLP0006)
