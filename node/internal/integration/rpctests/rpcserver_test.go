...
func testGetBlockCount(ctx context.Context, r *vgldtest.Harness, t *testing.T) {
...
func testGetBlockHash(ctx context.Context, r *vgldtest.Harness, t *testing.T) {
	// Create a new block connecting to the current tip.
	generatedBlockHashes, err := r.Node.Generate(ctx, 1)
	if err != nil {
		t.Fatalf("Unable to generate block: %v", err)
...
func TestRpcServer(t *testing.T) {
	...
	harness, err := vgldtest.New(t, chaincfg.RegNetParams(), nil, args)
	if err != nil {
	...
	tests := []struct {
		name string
		f    func(context.Context, *vgldtest.Harness, *testing.T)
	}{
		{
			f:    testGetBestBlock,
