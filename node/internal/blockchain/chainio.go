...
func serializeSizeForMinimalOutputs(tx *VGLutil.Tx) int {
...
func putTxToMinimalOutputs(target []byte, tx *VGLutil.Tx) int {
...
// already there.
func dbMaybeStoreBlock(dbTx database.Tx, block *VGLutil.Block) error {
...
// The amount is never encoded into spent transaction outputs in Vigil
...
func decodeSpentTxOut(serialized []byte, stxo *spentTxOut, amount int64,
	...
	// since in Vigil we only need pkScript at most due to fraud proofs
...
// txouts.
func dbFetchSpendJournalEntry(dbTx database.Tx, block *VGLutil.Block, isTreasuryEnabled bool) ([]spentTxOut, error) {
...
func (b *BlockChain) createChainState() error {
	...
	genesisBlock := VGLutil.NewBlock(b.chainParams.GenesisBlock)
...
// block for the provided node, deserialize it, and return a VGLutil.Block.
func dbFetchBlockByNode(dbTx database.Tx, node *blockNode) (*VGLutil.Block, error) {
	...
	block, err := VGLutil.NewBlockFromBytes(blockBytes)
	if err != nil {
		return nil, err
	}
