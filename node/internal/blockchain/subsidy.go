...
func blockOneCoinbasePaysTokens(tx *VGLutil.Tx, params *chaincfg.Params) error {
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		if txOut.Value != ledgerEntry.Amount {
			str := fmt.Sprintf("block one output %d generates %v instead of "+
				"required %v", i, VGLutil.Amount(txOut.Value),
				VGLutil.Amount(ledgerEntry.Amount))
			return ruleError(ErrBlockOneOutputs, str)
		}
	}
...
func coinbasePaysTreasuryAddress(subsidyCache *standalone.SubsidyCache, tx *VGLutil.Tx, height int64, voters uint16, params *chaincfg.Params, isTreasuryEnabled bool) error {
	...
	if orgSubsidy != treasuryOutput.Value {
		str := fmt.Sprintf("treasury output amount is %s instead of %s",
			VGLutil.Amount(treasuryOutput.Value), VGLutil.Amount(orgSubsidy))
		return ruleError(ErrNoTreasury, str)
	}
...
// the payout.
func checkTreasuryBase(subsidyCache *standalone.SubsidyCache, tx *VGLutil.Tx, height int64, voters uint16, params *chaincfg.Params) error {
	// Treasury subsidy only applies from block 2 onwards.
	if height <= 1 {
		return nil
	}
...
func calculateAddedSubsidy(block, parent *VGLutil.Block) int64 {
	var subsidy int64
