...
func TestTSpendLegacyExpendituresPolicy(t *testing.T) {
	...
	smallTSpend := g.CreateTreasuryTSpend(privKey, []chaingen.AddressAmountTuple{
		{
			Amount: VGLutil.Amount(uint64(smallTSpendAmount - smallTSpendFee)),
		},
	},
		VGLutil.Amount(smallTSpendFee), expiry)
	...
	for i := 0; i < nbTests; i++ {
		tspends[i] = g.CreateTreasuryTSpend(privKey, []chaingen.AddressAmountTuple{
			{
				Amount: VGLutil.Amount(uint64(testCases[i].amount) - tspendFee),
			},
		},
			VGLutil.Amount(tspendFee), expiry)
		h := tspends[i].TxHash()
	...
	tspend := g.CreateTreasuryTSpend(privKey, []chaingen.AddressAmountTuple{
		{
			Amount: VGLutil.Amount(uint64(expendBootstrap) - tspendFee),
		},
	},
		VGLutil.Amount(tspendFee), expiry)
	h = tspend.TxHash()
...
// TestTSpendExpendituresPolicyVGLP0007 performs tests against the treasury
...
//     done by VGLP0007)
func TestTSpendExpendituresPolicyVGLP0007(t *testing.T) {
	t.Parallel()
	...
	// Now we'll test that VGLP0007 fixes the issue with locked maximum
	// expenditure.
	//
