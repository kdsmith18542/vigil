...
func replaceCoinbase(b *wire.MsgBlock) {
	...
	retTx := VGLutil.NewTx(treasurybaseTx)
...
func TestTSpendVoteCount(t *testing.T) {
	...
	tspend := g.CreateTreasuryTSpend(privKey, []chaingen.AddressAmountTuple{{
		Amount: VGLutil.Amount(tspendAmount - tspendFee),
	}}, VGLutil.Amount(tspendFee), expiry)
...
func TestExpendituresReorg(t *testing.T) {
	...
	tspendFee := uint64(0)
	tspend := g.CreateTreasuryTSpend(privKey, []chaingen.AddressAmountTuple{{
		Amount: VGLutil.Amount(uint64(tspendAmount) - tspendFee),
	}}, VGLutil.Amount(tspendFee), expiry)
	tspendHash := tspend.TxHash()

	// Generate a TAdd for some amount.
	taddAmount := int64(1701)
	taddFee := VGLutil.Amount(0)
	tadd := g.CreateTreasuryTAdd(&outs[0], VGLutil.Amount(taddAmount), taddFee)
...
func TestSpendableTreasuryTxs(t *testing.T) {
	...
	taddFee := VGLutil.Amount(0)
	tadd1 := g.CreateTreasuryTAddChange(&taddOut1, VGLutil.Amount(taddAmount),
	...
	tadd2 := g.CreateTreasuryTAddChange(&outs[0], VGLutil.Amount(taddAmount),
		taddFee, addP2shAddr)
	...
	sig, err := sign.SignatureScript(tx, 0, tspend.TxOut[1].PkScript,
		txscript.SigHashAll, spendPrivKey.Serialize(), VGLec.STEcdsaSecp256k1,
		true)
	if err != nil {
	...
	sig, err = sign.SignatureScript(tx, 2, tadd1.TxOut[1].PkScript,
		txscript.SigHashAll, addPrivKey.Serialize(), VGLec.STEcdsaSecp256k1,
		true)
	if err != nil {
		t.Fatalf("unable to generate sig: %v", err)
	}
...
func TestTSpendSignature(t *testing.T) {
	...
	tspend1 := g.CreateTreasuryTSpend(privKey, []chaingen.AddressAmountTuple{
		{
			Amount: VGLutil.Amount(tspendAmount1 - tspendFee),
		},
	},
		VGLutil.Amount(tspendFee), expiry)
	...
	tspend2 := g.CreateTreasuryTSpend(privKey2, []chaingen.AddressAmountTuple{
		{
			Amount: VGLutil.Amount(tspendAmount2 - tspendFee),
		},
	},
		VGLutil.Amount(tspendFee), expiry)
...
func createTAdd(spend chaingen.SpendableOut, changeDivisor VGLutil.Amount, params *chaincfg.Params) func(b *wire.MsgBlock) {
...
func TestTSpendFirstTVICorner(t *testing.T) {
	...
	tspend := newFakeCreateTSpend(privKey, []VGLutil.Amount{100000000}, 1,
		expiry)
	tspendHash := tspend.TxHash()
...
func TestTreasuryInRegularTree(t *testing.T) {
	...
	tspend := g.CreateTreasuryTSpend(privKey, []chaingen.AddressAmountTuple{{
		Amount: VGLutil.Amount(tspendAmount - tspendFee),
	}}, VGLutil.Amount(tspendFee), 78) // Expiry check not hit.
...
func TestTSpendVoteCountSynthetic(t *testing.T) {
	...
	tests := []struct {
		...
		set: func(bc *BlockChain, node *blockNode) {
			...
			bc.addRecentBlock(VGLutil.NewBlock(mblk))
		},
	}, {
		name:             "All no",
		numNodes:         int64(end),
	...
	for _, test := range tests {
		...
		bc.recentBlocks = lru.NewMap[chainhash.Hash, *VGLutil.Block](
		...
		tv, err := bc.tSpendCountVotes(node.parent, VGLutil.NewTx(tspend))
		if err != nil {
			t.Fatal(err)
		}
		if tv.start != start {
		...
		// Check error
		err = bc.checkTSpendHasVotes(node.parent, VGLutil.NewTx(tspend))
		if test.failure {
			if err == nil {
...
func TestTSpendTooManyTAdds(t *testing.T) {
	...
	for i := 0; i < maxNbTadds+1; i++ {
		tadd := g.CreateTreasuryTAdd(&oldOuts[i], VGLutil.Amount(taddAmount), taddFee)
		tadd.Version = wire.TxVersionTreasury
