...
// calcNextKawpowDiff calculates the required difficulty for the block AFTER
// the passed previous block node based on the KawPoW difficulty retarget rules
// using ASERT (Absolutely Scheduled Exponentially Rising Targets).
//
...
func (b *BlockChain) calcNextKawpowDiff(prevNode *blockNode, newBlockTime time.Time) uint32 {
	...
	// Find the KawPoW anchor block for ASERT calculation
	kawpowAnchor := b.kawpowWorkDiffAnchor(prevNode)

	// Calculate the next difficulty using ASERT relative to the anchor
	return b.calcNextKawpowDiffFromAnchor(prevNode, kawpowAnchor)
}

// calcNextKawpowDiffFromAnchor calculates the required difficulty for the block
// AFTER the passed previous block node relative to the given anchor block based
// on the KawPoW difficulty retarget rules using ASERT.
//
// This function is safe for concurrent access.
func (b *BlockChain) calcNextKawpowDiffFromAnchor(prevNode *blockNode, kawpowAnchor *blockNode) uint32 {
	// Calculate the time and height deltas as the difference between the
	// provided block and the KawPoW anchor block.
...
// kawpowWorkDiffAnchor returns the block to treat as the anchor block for the
// purposes of determining how far ahead or behind the ideal schedule the
// provided block is when calculating the KawPoW target difficulty for the block
// AFTER the passed previous block node.
//
// This function MUST only be called with the KawPoW agenda active
// after having gone through a vote.
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) kawpowWorkDiffAnchor(prevNode *blockNode) *blockNode {
	// Use the previously cached anchor when it exists and is actually an
	// ancestor of the provided block.
	if b.kawpowWorkDiffAnchorCache != nil {
		anchor := b.kawpowWorkDiffAnchorCache
		if prevNode.IsAncestorOf(anchor) {
			return anchor
		}
	}

	// Find the most recent block where the KawPoW agenda became active.
	// This will be used as the anchor for ASERT calculations.
	anchor := prevNode
	for anchor != nil && anchor.parent != nil {
		isActive, err := b.isKawpowAgendaActive(anchor.parent)
		if err != nil {
			// If there's an error checking agenda status, use the current block
			break
		}
		if !isActive {
			// Found the activation point - use this block as anchor
			break
		}
		anchor = anchor.parent
	}

	// Cache the anchor for future use
	b.kawpowWorkDiffAnchorCache = anchor
	return anchor
}
...
// rules.
// It now also considers a hypothetical KawPoW agenda.
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) calcNextRequiredDifficulty(prevNode *blockNode, newBlockTime time.Time) (uint32, error) {
	// Check for KawPoW agenda first
	isKawpowActive, err := b.isKawpowAgendaActive(prevNode)
	if err != nil {
		return 0, err
	}
	if isKawpowActive {
	...
	isBlake3Active, err := b.isBlake3PowAgendaActive(prevNode)
	if err != nil {
		return 0, err
	}
	if isBlake3Active {
...
// weighted averages.
//
// NOTE: This is the original stake difficulty algorithm that was used at Vigil
// launch.
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) calcNextRequiredStakeDifficultyV1(curNode *blockNode) int64 {
	...
	// Vigil parameters, but might do weird things if you use custom
	// parameters.
	if curNode == nil ||
...
// algorithm defined in VGLP0001 by pretending the provided number of tickets
