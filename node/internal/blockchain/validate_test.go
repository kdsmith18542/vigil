...
func TestBlockchainSpendJournal(t *testing.T) {
	...
	for i := 1; i < finalIdx1+1; i++ {
		bl, err := VGLutil.NewBlockFromBytes(blockChain[int64(i)])
		if err != nil {
...
func TestCheckBlockSanity(t *testing.T) {
	...
	block := VGLutil.NewBlock(&badBlock)
...
func TestModifiedSubsidySplitSemantics(t *testing.T) {
	...
	replaceCoinbaseSubsidy := func(b *wire.MsgBlock) {
		...
		const numVotes = 5
		const splitVariation = standalone.SSVVGLP0010
	...
	replaceVoteSubsidies := func(b *wire.MsgBlock) {
		...
		chaingen.ReplaceVoteSubsidies(VGLutil.Amount(voteSubsidy))(b)
	}
...
func TestModifiedSubsidySplitR2Semantics(t *testing.T) {
	...
	replaceCoinbaseSubsidy := func(b *wire.MsgBlock) {
		...
		// Calculate the modified pow subsidy along with the treasury subsidy.
		const numVotes = 5
		const splitVariation = standalone.SSVVGLP0012
		height := int64(b.Header.Height)
		trsySubsidy := cache.CalcTreasurySubsidy(height, numVotes, noTreasury)
		powSubsidy := cache.CalcWorkSubsidyV3(height, numVotes, splitVariation)
