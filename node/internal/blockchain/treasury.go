...
func (b *BlockChain) dbPutTreasuryBalance(dbTx database.Tx, block *VGLutil.Block, node *blockNode) error {
...
// database.
func (b *BlockChain) dbPutTSpend(dbTx database.Tx, block *VGLutil.Block) error {
	hash := block.Hash()
	msgBlock := block.MsgBlock()
...
// maxTreasuryExpenditureVGLP0006 returns the maximum amount of funds that can
...
// as defined in VGLP0006.
//
// See notes on maxTreasuryExpenditure().
func (b *BlockChain) maxTreasuryExpenditureVGLP0006(preTVINode *blockNode) (int64, error) {
...
// maxTreasuryExpenditureVGLP0007 returns the maximum amount of funds that can
// be spent from the treasury at the block after the provided node.
//
// This is code that was activated as part of the 'reverttreasurypolicy'
// consensus upgrade as defined in VGLP0007.
//
// See notes on maxTreasuryExpenditure().
func (b *BlockChain) maxTreasuryExpenditureVGLP0007(preTVINode *blockNode) (int64, error) {
	// The expenditure policy check is roughly speaking:
	//
	//     "The sum of tspends inside an expenditure window cannot exceed
	...
	trsyLog.Tracef("  maxTSpendExpenditureVGLP0007: spent %d, "+
...
func (b *BlockChain) maxTreasuryExpenditure(preTVINode *blockNode) (int64, error) {
	...
	if isRevertPolicyActive {
		return b.maxTreasuryExpenditureVGLP0007(preTVINode)
	}
	return b.maxTreasuryExpenditureVGLP0006(preTVINode)
}
...
func (b *BlockChain) tSpendCountVotes(prevNode *blockNode, tspend *VGLutil.Tx) (*tspendVotes, error) {
...
func (b *BlockChain) TSpendCountVotes(blockHash *chainhash.Hash, tspend *VGLutil.Tx) (yesVotes, noVotes int64, err error) {
	b.index.RLock()
...
// on a TVI.
func (b *BlockChain) checkTSpendHasVotes(prevNode *blockNode, tspend *VGLutil.Tx) error {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CheckTSpendHasVotes(prevHash chainhash.Hash, tspend *VGLutil.Tx) error {
	prevNode := b.index.LookupNode(&prevHash)
	if prevNode == nil {
