...
type testChain struct {
	...
	treasuryActive   bool
	keyedByHeight    map[int64]*VGLutil.Block
	keyedByHash      map[chainhash.Hash]*VGLutil.Block
	orphans          map[chainhash.Hash]*VGLutil.Block
...
func newTestChain() (*testChain, error) {
	tc := &testChain{
		keyedByHeight:    make(map[int64]*VGLutil.Block),
		keyedByHash:      make(map[chainhash.Hash]*VGLutil.Block),
		orphans:          make(map[chainhash.Hash]*VGLutil.Block),
		removedSpendDeps: make(map[chainhash.Hash][]string),
	}
	genesis := VGLutil.NewBlock(chaincfg.SimNetParams().GenesisBlock)
	return tc, tc.AddBlock(genesis)
}

// AddBlock extends the chain with the provided block.
func (tc *testChain) AddBlock(blk *VGLutil.Block) error {
	tc.mtx.Lock()
	defer tc.mtx.Unlock()
...
// current chain tip.
func (tc *testChain) RemoveBlock(blk *VGLutil.Block) error {
	tc.mtx.Lock()
	defer tc.mtx.Unlock()
...
func (tc *testChain) BlockByHash(hash *chainhash.Hash) (*VGLutil.Block, error) {
	tc.mtx.Lock()
	defer tc.mtx.Unlock()
...
func addBlock(t *testing.T, chain *testChain, gen *chaingen.Generator, name string) *VGLutil.Block {
