...
func TestFetchUtxoView(t *testing.T) {
	...
	// should be included.
	fetchUtxoViewTipApproved := func(tx *VGLutil.Tx) (*UtxoViewpoint, error) {
	...
	fetchUtxoViewTipDisapproved := func(tx *VGLutil.Tx) (*UtxoViewpoint, error) {
	...
	testInputsSpent := func(view *UtxoViewpoint, tx *VGLutil.Tx, spent bool) {
		t.Helper()
	...
	spendB0Tx1Out0 := VGLutil.NewTx(g.CreateSpendTx(&b0Tx1Out0, fee))
	...
	b0Tx1 := VGLutil.NewTx(b0.Transactions[1])
	...
	spendB1aTx1Out0 := VGLutil.NewTx(g.CreateSpendTx(&b1aTx1Out0, fee))
