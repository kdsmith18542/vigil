...
func (b *BlockChain) maybeAcceptBlockHeader(header *wire.BlockHeader, checkHeaderSanity bool) (*blockNode, error) {
	...
	if checkHeaderSanity {
		err := checkBlockHeaderSanity(header, b.chainParams, BFNone)
		if err != nil {
			return nil, err
		}
	}
...
//
// This function MUST be called with the chain lock held (for writes).
func (b *BlockChain) maybeAcceptBlockData(node *blockNode, block *VGLutil.Block, flags BehaviorFlags) ([]*blockNode, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) ProcessBlock(block *VGLutil.Block) (int64, error) {
