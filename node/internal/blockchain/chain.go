...
// Package blockchain implements Vigil block handling and chain selection
// rules.
package blockchain
...
// BlockChain provides functions for working with the Vigil block chain.  It
...
type BlockChain struct {
	...
	recentBlocks        *lru.Map[chainhash.Hash, *VGLutil.Block]
	...
	// point to use for the difficulty algorithm defined in VGLP0011 before the
	...
	// difficulty algorithm defined in VGLP0011.
	//
	...
	// kawpowWorkDiffAnchorCache houses a cached anchor point to use for the
	// difficulty algorithm defined in VGLP0011.
	//
	// It is only set when the kawpow proof of work agenda has been determined
	// to be active and will be the block just prior to the activation of the
	// agenda.  It will not be set for networks where the agenda is always
	// active such as the simulation network.
	kawpowWorkDiffAnchorCache *blockNode
...
// newBlockChain returns a new BlockChain instance with all fields initialized.
// This includes initializing the kawpowWorkDiffAnchorCache to nil.
func newBlockChain(db database.DB, chainParams *chaincfg.Params, timeSource MedianTimeSource,
	notifications NotificationCallback, sigCache *txscript.SigCache,
	indexSubscriber *indexers.IndexSubscriber, interrupt <-chan struct{},
	utxoCache UtxoCacher) (*BlockChain, error) {

	// Initialize the chain state from the passed database.  When the db
	// does not yet contain any chain state, both it and the chain state
	// will be initialized to contain only the genesis block.
	chain := &BlockChain{
		assumeValid:              *zeroHash,

		expectedBlocksInTwoWeeks: int64(chainParams.TargetTimespan / chainParams.TargetTimePerBlock),
		deploymentData:           make(map[string]deploymentInfo),
		minKnownWork:             new(uint256.Uint256),
		minTestNetTarget:         new(big.Int),
		minTestNetDiffBits:       0,
		kawpowWorkDiffAnchorCache: nil,
		db:                       db,
		dbInfo:                   &databaseInfo{},
		...
		sigCache:                 sigCache,
		indexSubscriber:          indexSubscriber,
		interrupt:                interrupt,
		utxoCache:                utxoCache,
		subsidyCache:             standalone.NewSubsidyCache(chainParams),
		index:                    newBlockIndex(db),
		bestChain:                newChainView(nil),
		recentBlocks:             lru.NewMap[chainhash.Hash, *VGLutil.Block](recentBlockCacheSize),
		recentContextChecks:      lru.NewSet[chainhash.Hash](contextCheckCacheSize),
		disapprovedView:         NewUtxoViewpoint(utxoCache),
...
// StakeVersions is a condensed form of a VGLutil.Block that is used to prevent
...
//
// This function is safe for concurrent access.
func (b *BlockChain) addRecentBlock(block *VGLutil.Block) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) lookupRecentBlock(hash *chainhash.Hash) (*VGLutil.Block, bool) {
...
func (b *BlockChain) fetchMainChainBlockByNode(node *blockNode) (*VGLutil.Block, error) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) fetchBlockByNode(node *blockNode) (*VGLutil.Block, error) {
	// Attempt to load the block from the recent block cache.
	block, ok := b.lookupRecentBlock(&node.hash)
	if ok {
		return block, nil
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block, parent *VGLutil.Block, view *UtxoViewpoint, stxos []spentTxOut, hdrCommitments *headerCommitmentData) error {
	...
	// Reset the cached KawPoW work difficulty anchor since the chain has
	// reorged and it is no longer valid.
	b.kawpowWorkDiffAnchorCache = nil
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block, parent *VGLutil.Block, view *UtxoViewpoint) error {
...
func countSpentRegularOutputs(block *VGLutil.Block) int {
...
func countSpentStakeOutputs(block *VGLutil.Block) int {
	var numSpent int
...
func countSpentOutputs(block *VGLutil.Block) int {
...
func (b *BlockChain) loadOrCreateFilter(block *VGLutil.Block, view *UtxoViewpoint) (*gcs.FilterV2, error) {
...
func (b *BlockChain) reorganizeChainInternal(target *blockNode) error {
	...
	var nextBlockToDetach *VGLutil.Block
	...
	var prevBlockAttached *VGLutil.Block
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHash(hash *chainhash.Hash) (*VGLutil.Block, error) {
	node := b.index.LookupNode(hash)
...
//
// This function is safe for concurrent access.
func (b *BlockChain) BlockByHeight(height int64) (*VGLutil.Block, error) {
...
func validateDeploymentChoices(voteParams *chaincfg.Vote) error {
	...
	if VGLutil.IsFlagSet16(voteParams.Mask, VGLutil.BlockValid) {
...
func newRecentBlocksCache() *lru.Map[chainhash.Hash, *VGLutil.Block] {
	return lru.NewMap[chainhash.Hash, *VGLutil.Block](recentBlockCacheSize)
}
