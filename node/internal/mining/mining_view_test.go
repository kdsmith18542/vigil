...
func TestMiningView(t *testing.T) {
	...
	allTxns := []*VGLutil.Tx{txB, txA, txC, txD, txE}
	...
	tests := []struct {
		...
		expectedSigOps       int
		ancestors            []*VGLutil.Tx
		descendants          map[chainhash.Hash]*VGLutil.Tx
		orderedAncestors     [][]*VGLutil.Tx
	}{{
		name:                 "Tx A",
		subject:              txA,
		...
		expectedSigOps: txATotalSigOps + txBTotalSigOps +
			txCTotalSigOps,
		ancestors:   []*VGLutil.Tx{txA, txB, txC},
		descendants: map[chainhash.Hash]*VGLutil.Tx{},
		orderedAncestors: [][]*VGLutil.Tx{
			{txA, txB, txC},
			{txA, txC, txB},
