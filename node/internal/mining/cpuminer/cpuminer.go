...
type Config struct {
	...
	ProcessBlock func(*VGLutil.Block) error
...
func (m *CPUMiner) submitBlock(block *VGLutil.Block, isBlake3PowActive bool) bool {
	...
	isKawpowActive, err := m.cfg.IsKawpowAgendaActive(&block.MsgBlock().Header.PrevBlock)
	if err != nil {
		log.Errorf("Error checking KawPoW agenda status: %v", err)
...
// solveBlockKawpowCPU attempts to find a KawPoW solution for the given block header.
func (m *CPUMiner) solveBlockKawpowCPU(ctx context.Context, header *wire.BlockHeader, stats *speedStats) bool {
	log.Infof("Attempting to solve block %d with KawPoW (CPU Miner)", header.Height)

	// Convert bits to target
	target := standalone.CompactToBig(header.Bits)
	if target == nil {
		log.Errorf("Unable to convert diff bits %08x to target for KawPoW", header.Bits)
		return false
	}

	// Convert target to 32-byte little-endian slice
	targetBytes := make([]byte, 32)
	targetBigEndian := target.Bytes()
	// Copy to the end of targetBytes (little-endian)
	for i := 0; i < len(targetBigEndian) && i < 32; i++ {
		targetBytes[31-i] = targetBigEndian[len(targetBigEndian)-1-i]
	}

	// Create nonce range (placeholder values)
	...
	nonceRangeSize := nonceRange[1] - nonceRange[0]
	height := uint64(header.Height)
	result, found, err := kawpow.Mine(headerHash, startNonce, nonceRangeSize, height, targetBytes)
	if err != nil {
		if !errors.Is(err, context.Canceled) {
			log.Errorf("Error searching for KawPoW solution: %v", err)
		}
		return false
	}
	...
	if found && result != nil {
		...
		log.Infof("KawPoW solution found for block %d: Nonce %x, MixDigest %x, Hash %x",
			header.Height, result.Nonce, result.MixHash, result.Hash)
		return true
	}
