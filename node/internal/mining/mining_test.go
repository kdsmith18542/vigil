...
func TestNewBlockTemplate(t *testing.T) {
	...
	const numVotes = 5
	tickets := make([]*VGLutil.Tx, numVotes)
	...
	votes := make([]*VGLutil.Tx, numVotes)
	for i, ticket := range tickets {
	...
	block := VGLutil.NewBlock(blockTemplate.Block)
...
func TestNewBlockTemplateAutoRevocations(t *testing.T) {
	...
	// regular transactions.
	const numTickets = 10
	tickets := make([]*VGLutil.Tx, numTickets)
