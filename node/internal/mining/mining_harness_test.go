...
func (c *fakeChain) AddBlock(block *VGLutil.Block) {
...
func (c *fakeChain) BlockByHash(hash *chainhash.Hash) (*VGLutil.Block, error) {
...
// requirement.
func (c *fakeChain) CheckConnectBlockTemplate(block *VGLutil.Block) error {
...
// prevHash block.
func (c *fakeChain) CheckTSpendHasVotes(prevHash chainhash.Hash, tspend *VGLutil.Tx) error {
...
func (c *fakeChain) FetchUtxoView(tx *VGLutil.Tx, treeValid bool) (*blockchain.UtxoViewpoint, error) {
...
type fakeTxSource struct {
	...
	outpoints       map[wire.OutPoint]*VGLutil.Tx
	orphans         map[chainhash.Hash]*VGLutil.Tx
	orphansByPrev   map[wire.OutPoint]map[chainhash.Hash]*VGLutil.Tx
	staged          map[chainhash.Hash]*VGLutil.Tx
	stagedOutpoints map[wire.OutPoint]*VGLutil.Tx
	votes           map[chainhash.Hash][]VoteDesc
	tspends         map[chainhash.Hash]*VGLutil.Tx
...
// given transaction.
func (p *fakeTxSource) CountTotalSigOps(tx *VGLutil.Tx, txType stake.TxType) (int, error) {
...
// is provided.
func (p *fakeTxSource) fetchRedeemers(outpoints map[wire.OutPoint]*VGLutil.Tx,
	tx *VGLutil.Tx) []*VGLutil.Tx {
...
func (p *fakeTxSource) removeOrphan(tx *VGLutil.Tx, removeRedeemers bool) {
...
func (p *fakeTxSource) removeOrphanDoubleSpends(tx *VGLutil.Tx) {
	msgTx := tx.MsgTx()
...
func (p *fakeTxSource) addTransaction(tx *VGLutil.Tx, txType stake.TxType, height int64, fee int64, totalSigOps int) {
...
func (p *fakeTxSource) insertVote(ssgen *VGLutil.Tx) {
	...
	vote := VGLutil.IsFlagSet16(voteBits, VGLutil.BlockValid)
	voteTx := VoteDesc{
		VoteHash:       *voteHash,
...
// pool.
func (p *fakeTxSource) stageTransaction(tx *VGLutil.Tx) {
	p.staged[*tx.Hash()] = tx
	for _, txIn := range tx.MsgTx().TxIn {
...
func (p *fakeTxSource) removeStagedTransaction(stagedTx *VGLutil.Tx) {
...
func (p *fakeTxSource) RemoveTransaction(tx *VGLutil.Tx, removeRedeemers,
...
// main pool.
func (p *fakeTxSource) hasPoolInput(tx *VGLutil.Tx) bool {
	for _, txIn := range tx.MsgTx().TxIn {
...
func (p *fakeTxSource) MaybeAcceptDependents(tx *VGLutil.Tx) []*VGLutil.Tx {
	var acceptedTxns []*VGLutil.Tx
...
// source.
func (p *fakeTxSource) maybeAcceptTransaction(tx *VGLutil.Tx, isNew bool) ([]*chainhash.Hash, error) {
	msgTx := tx.MsgTx()
	txHash := tx.Hash()
...
func (p *fakeTxSource) processOrphans(acceptedTx *VGLutil.Tx) []*VGLutil.Tx {

	var acceptedTxns []*VGLutil.Tx

	// Start with processing at least the passed transaction.
	processList := []*VGLutil.Tx{acceptedTx}
...
// fake tx source.
func (p *fakeTxSource) ProcessTransaction(tx *VGLutil.Tx) ([]*VGLutil.Tx, error) {
	...
	if len(missingParents) == 0 {
		...
		acceptedTxs := make([]*VGLutil.Tx, len(newTxs)+1)
...
// transaction pool.
func (p *fakeTxSource) fetchInputUtxos(tx *VGLutil.Tx, isTreasuryEnabled bool) (*blockchain.UtxoViewpoint, error) {
...
func (m *miningHarness) GetKey(addr stdaddr.Address) ([]byte, VGLec.SignatureType, bool, error) {
...
func (m *miningHarness) CreateCoinbaseTx(blockHeight int64, numOutputs uint32) (*VGLutil.Tx, error) {
...
func (m *miningHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*VGLutil.Tx, error) {
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		sigScript, err := sign.SignatureScript(tx, 0, m.payScript,
			txscript.SigHashAll, m.signKey, VGLec.STEcdsaSecp256k1, true)
		if err != nil {
			return nil, err
		}
		tx.TxIn[0].SignatureScript = sigScript

		txChain = append(txChain, VGLutil.NewTx(tx))
...
// output.
func (m *miningHarness) CreateTx(out spendableOutput) (*VGLutil.Tx, error) {
...
// testing.
func (m *miningHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32, mungers ...func(*wire.MsgTx)) (*VGLutil.Tx, error) {
...
func (m *miningHarness) CreateTicketPurchase(sourceTx *VGLutil.Tx, cost int64) (*VGLutil.Tx, error) {
...
// the opportunity to modify the transaction which is especially useful for
// testing.
func (m *miningHarness) CreateVote(ticket *VGLutil.Tx, mungers ...func(*wire.MsgTx)) (*VGLutil.Tx, error) {
...
// CountTotalSigOps returns the total number of signature operations for the
// given transaction.
func (m *miningHarness) CountTotalSigOps(tx *VGLutil.Tx) (int, error) {
	txType := stake.DetermineTxType(tx.MsgTx())
	return m.txSource.CountTotalSigOps(tx, txType)
}

// AddTransactionToTxSource adds the given transaction to the tx source.
func (m *miningHarness) AddTransactionToTxSource(tx *VGLutil.Tx) ([]*VGLutil.Tx, error) {
	return m.txSource.ProcessTransaction(tx)
}

// RemoveTransactionFromTxSource removes the given transaction from the tx
// source.
func (m *miningHarness) RemoveTransactionFromTxSource(tx *VGLutil.Tx, removeRedeemers bool) {
...
func (m *miningHarness) AddFakeUTXO(tx *VGLutil.Tx, blockHeight int64, blockIndex uint32, isTreasuryAgendaActive bool) {
...
func newMiningHarness(chainParams *chaincfg.Params) (*miningHarness, []spendableOutput, error) {
	...
	policy := &Policy{
		...
		TxMinFreeFee:     VGLutil.Amount(1e4),
	...
	txSource := &fakeTxSource{
		...
		outpoints:       make(map[wire.OutPoint]*VGLutil.Tx),
		orphans:         make(map[chainhash.Hash]*VGLutil.Tx),
		orphansByPrev:   make(map[wire.OutPoint]map[chainhash.Hash]*VGLutil.Tx),
		staged:          make(map[chainhash.Hash]*VGLutil.Tx),
		stagedOutpoints: make(map[wire.OutPoint]*VGLutil.Tx),
		votes:           make(map[chainhash.Hash][]VoteDesc),
		tspends:         make(map[chainhash.Hash]*VGLutil.Tx),
	}
	...
	// given transaction,
	forEachRedeemer := func(tx *VGLutil.Tx, f func(redeemerTx *TxDesc)) {
		prevOut := wire.OutPoint{Hash: *tx.Hash(), Tree: tx.Tree()}
