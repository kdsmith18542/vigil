...
type Config struct {
	...
	// BlockTemplateGenerator provides a template of a block to be solved.
	...
	BlockByHash func(hash *chainhash.Hash) (*VGLutil.Block, error)
	...
	CheckTransactionInputs func(tx *VGLutil.Tx, txHeight int64,
	...
	CheckTSpendHasVotes func(prevHash chainhash.Hash, tspend *VGLutil.Tx) error
	...
	// transaction.
	CountSigOps func(tx *VGLutil.Tx, isCoinBaseTx bool, isSSGen bool, isTreasuryEnabled bool) int
	...
	FetchUtxoView func(tx *VGLutil.Tx, includeRegularTxns bool) (*blockchain.UtxoViewpoint, error)
	...
	IsFinalizedTransaction func(tx *VGLutil.Tx, blockHeight int64, blockTime time.Time) bool
	...
	ValidateTransactionScripts func(tx *VGLutil.Tx,
...
func containsTxIns(txs []*VGLutil.Tx, tx *VGLutil.Tx) bool {
...
type BlockTemplate struct {
	...
	// MixHash is the KawPoW mix hash for the block.
	MixHash chainhash.Hash
	// Nonce is the KawPoW nonce for the block.
	Nonce uint64
...
// BlockTemplateGenerator provides a template of a block to be solved.
// This interface is used to allow the CPU miner to be decoupled from
// the blockchain.  It is implemented by the blockchain package.
type BlockTemplateGenerator interface {
	NewBlockTemplate(payToAddress stdaddr.Address) (*BlockTemplate, error)
	GetBlockTemplate(payToAddress stdaddr.Address) (*BlockTemplate, error)
	GetBlockTemplateKawPow(payToAddress stdaddr.Address) (*BlockTemplate, error)
	UpdateBlockTemplate(template *BlockTemplate) error
...
// can not be found.
func txIndexFromTxList(hash chainhash.Hash, list []*VGLutil.Tx) int {
	for i, tx := range list {
		h := tx.Hash()
		if hash == *h {
			return i
...
// regular and stake transaction trees in accordance with VGLP0005.
...
func createCoinbaseTx(subsidyCache *standalone.SubsidyCache,
	...
	subsidySplitVariant standalone.SubsidySplitVariant) *VGLutil.Tx {
...
func createTreasuryBaseTx(subsidyCache *standalone.SubsidyCache, nextBlockHeight int64, voters uint16) (*VGLutil.Tx, error) {
...
func spendTransaction(utxoView *blockchain.UtxoViewpoint, tx *VGLutil.Tx,
...
func logSkippedDeps(tx *VGLutil.Tx, deps []*TxDesc) {
	if deps == nil {
		return
	}
...
func (g *BlkTmplGenerator) maybeInsertStakeTx(stx *VGLutil.Tx, treeValid bool, isTreasuryEnabled bool) bool {
	missingInput := false
...
func (g *BlkTmplGenerator) createRevocationFromTicket(ticketHash *chainhash.Hash,
	...
	const revocationTxFee = VGLutil.Amount(0)
	...
	revocationTx := VGLutil.NewTx(revocationMsgTx)
...
// the need to have configured address can be avoided.
//
// This function is a wrapper around NewBlockTemplate that simply calls it.
func (g *BlkTmplGenerator) GetBlockTemplate(payToAddress stdaddr.Address) (*BlockTemplate, error) {
	return g.NewBlockTemplate(payToAddress)
}

// GetBlockTemplateKawPow returns a new block template that is ready to be solved
// using the transactions from the passed transaction source pool and a coinbase
// that either pays to the passed address if it is not nil, or a coinbase that
// is redeemable by anyone if the passed address is nil.  The nil address
// functionality is useful since there are cases such as the getblocktemplate
...
// the need to have configured address can be avoided.
//
// This function is a wrapper around NewBlockTemplateKawPow that simply calls it.
func (g *BlkTmplGenerator) GetBlockTemplateKawPow(payToAddress stdaddr.Address) (*BlockTemplate, error) {
	return g.NewBlockTemplateKawPow(payToAddress)
}

// NewBlockTemplateKawPow returns a new block template that is ready to be solved
// using the transactions from the passed transaction source pool and a coinbase
// that either pays to the passed address if it is not nil, or a coinbase that
// is redeemable by anyone if the passed address is nil.  The nil address
// functionality is useful since there are cases such as the getblocktemplate
...
// This function returns nil when there are not enough voters on any of the
// current top blocks to create a new block template.
func (g *BlkTmplGenerator) NewBlockTemplateKawPow(payToAddress stdaddr.Address) (*BlockTemplate, error) {
	...
	// Create a new block template and fill it with the relevant details
	// needed to generate a block.
	blockTemplate := &BlockTemplate{
		Block: wire.MsgBlock{
			Header: wire.BlockHeader{
				Version:    generatedBlockVersionTest,
				PrevBlock:  *best.Hash,
				Timestamp:  g.cfg.TimeSource.AdjustedTime(),
				Bits:       g.cfg.ChainParams.PowLimitBits,
				Nonce:      0,
				ExtraData:  g.cfg.Policy.ExtraData,
				StakeVersion: best.StakeVersion,
			},
		},
		Fees:            0,
		Mass:            0,
	...
	// Set the timestamp to the appropriate historical time if the mining time
	// offset is non-zero.
	if g.cfg.MiningTimeOffset != 0 {
		blockTemplate.Block.Header.Timestamp = blockTemplate.Block.Header.Timestamp.Add(
			-time.Second * time.Duration(g.cfg.MiningTimeOffset))
	}

	// Calculate the next required difficulty.  The difficulty must be calculated
	// here since it is based on the current best chain and is used to determine
	// the target difficulty for the new block.
	nextRequiredDifficulty, err := g.cfg.CalcNextRequiredDifficulty(best.Hash,
		blockTemplate.Block.Header.Timestamp)
	if err != nil {
		return nil, err
	}
	blockTemplate.Block.Header.Bits = nextRequiredDifficulty

	// Calculate the expected stake version for the new block.
	nextStakeVersion, err := g.cfg.CalcStakeVersionByHash(best.Hash)
	if err != nil {
		return nil, err
	}
	blockTemplate.Block.Header.StakeVersion = nextStakeVersion

	// Add a coinbase transaction to the block template.
	coinbaseTx, err := createCoinbaseTx(g.cfg.SubsidyCache, g.cfg.ChainParams,
		payToAddress, blockTemplate.Height, g.cfg.Policy.CoinbaseFlags)
	if err != nil {
		return nil, err
	}
	blockTemplate.AddTransaction(VGLutil.NewTx(coinbaseTx))

	// Calculate the KawPoW hash and mixhash.
	// This is a placeholder for actual KawPoW mining logic.
	// In a real scenario, this would involve a mining process to find a valid nonce and mixhash.
	// For now, we'll use dummy values or a simplified calculation.

	// Convert header to bytes for hashing.
	headerBytes, err := blockTemplate.Block.Header.Bytes()
	if err != nil {
		return nil, err
	}
	...
	headerHashChain := blockTemplate.Block.Header.BlockHash()

	// Convert chainhash.Hash to kawpow.Hash type.
	var headerHash kawpow.Hash
	copy(headerHash[:], headerHashChain[:])

	// Generate a dummy nonce for now. In a real scenario, this would be found through mining.
	nonce := uint64(0)

	// Create a dummy DAG for now. In a real scenario, this would be generated based on epoch.
	dag := kawpow.NewDAG(uint64(blockTemplate.Block.Header.Height) / kawpow.EPOCH_LENGTH)
	err = dag.Generate(uint64(blockTemplate.Block.Header.Height) / kawpow.EPOCH_LENGTH)
	if err != nil {
		return nil, err
	}
	...
	// Populate the block template with the calculated nonce and mixhash.
	...
	// template.
	numTxns := 0
	for _, tx := range g.cfg.TxSource.MiningDescs() {
		// Skip transactions that are not standard.
		if g.cfg.Policy.RejectNonStd && tx.Type == VGLutil.TxTypeRegular &&
			!g.cfg.Policy.Is  Standard(tx.Tx) {
			continue
		}

		// Skip transactions that are too large.
		if tx.Tx.MsgTx().SerializeSize() > g.cfg.Policy.BlockMaxSize/2 {
			continue
		}

		// Skip transactions that are too small.
		if tx.Tx.MsgTx().SerializeSize() < g.cfg.Policy.MinTxSize {
			continue
		}

		// Skip transactions that are not valid for the current block height.
		if !g.cfg.IsFinalizedTransaction(tx.Tx, blockTemplate.Height,
			blockTemplate.Block.Header.Timestamp) {
			continue
		}

		// Skip transactions that are not valid for the current block time.
		if tx.Tx.MsgTx().LockTime > uint32(blockTemplate.Block.Header.Timestamp.Unix()) {
			continue
		}

		// Skip transactions that are not valid for the current block version.
		if tx.Tx.MsgTx().Version > blockTemplate.Block.Header.Version {
			continue
		}

		// Skip transactions that are not valid for the current chain parameters.
		if !g.cfg.ChainParams.Is  ValidTx(tx.Tx) {
			continue
		}

		// Skip transactions that are not valid for the current network.
		if !g.cfg.ChainParams.Is  ValidNetwork(tx.Tx) {
			continue
		}

		// Skip transactions that are not valid for the current consensus rules.
		if !g.cfg.ChainParams.Is  ValidConsensus(tx.Tx) {
			continue
		}

		// Skip transactions that are not valid for the current block template.
		if !blockTemplate.AddTransaction(tx.Tx) {
			continue
		}
	...
	// Update the block template with the new transactions.
	blockTemplate.UpdateHeader()
	...
	// avoided.
	blockTxns := make([]*VGLutil.Tx, 0, len(sourceTxns))
...
nextPriorityQueueItem:
	...
	blockTxnsStake := make([]*VGLutil.Tx, 0, len(blockTxns))
	...
	// Vigil: We need to move this downwards because of the requirements
	...
	votes := make([]*VGLutil.Tx, 0, g.cfg.ChainParams.TicketsPerBlock)
	...
	if nextBlockHeight >= stakeValidationHeight {
		for _, tx := range blockTxns {
			...
			if stake.IsSSGen(msgTx) {
				txCopy := VGLutil.NewTxDeepTxIns(tx)
	...
	if nextBlockHeight < stakeValidationHeight {
		...
		for _, vb := range voteBitsVoters {
			if VGLutil.IsFlagSet16(vb, VGLutil.BlockValid) {
				voteYea++
			}
			totalVotes++
		}
	...
	blockTxnsRegular := make([]*VGLutil.Tx, 0, len(blockTxns)+1)
	...
	log.Debugf("Created new block template (%d transactions, %d stake "+
		...
		VGLutil.Amount(msgBlock.Header.SBits).ToCoin())
