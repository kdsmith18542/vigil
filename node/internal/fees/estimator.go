...
func (stats *Estimator) EstimateFee(targetConfs int32) (VGLutil.Amount, error) {
	stats.lock.RLock()
	...
	return VGLutil.Amount(rate), nil
}
...
func (stats *Estimator) AddMemPoolTransaction(txHash *chainhash.Hash, fee, size int64, txType stake.TxType) {
	...
	// minimum at values less than 0.001 VGL/KB. This is needed because due to
...
func (stats *Estimator) ProcessBlock(block *VGLutil.Block) error {
	stats.lock.Lock()
	defer stats.lock.Unlock()
