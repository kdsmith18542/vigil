...
import (
	...
	"github.com/vigilnetwork/vgl/container/apbf"
...
// blockMsg packages a Vigil block message and the peer it came from together
...
// invMsg packages a Vigil inv message and the peer it came from together
// so the event handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *Peer
...
// headersMsg packages a Vigil headers message and the peer it came from
...
// notFoundMsg packages a Vigil notfound message and the peer it came from
// together so the event handler has access to that information.
...
// txMsg packages a Vigil tx message and the peer it came from together
// so the event handler has access to that information.
type txMsg struct {
	tx    *VGLutil.Tx
	peer  *Peer
...
func (m *SyncManager) processBlock(block *VGLutil.Block) (int64, error) {
...
// queue.
func (m *SyncManager) OnTx(tx *VGLutil.Tx, peer *Peer, done chan struct{}) {
	select {
...
// queue.
func (m *SyncManager) OnBlock(block *VGLutil.Block, peer *Peer, done chan struct{}) {
	select {
...
func (m *SyncManager) ProcessBlock(block *VGLutil.Block) error {
