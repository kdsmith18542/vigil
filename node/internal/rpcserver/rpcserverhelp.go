...
var helpDescsEnUS = map[string]string{
	...
	"debuglevel--synopsis": "Dynamically changes the debug logging level.\n" +
		...
		"The valid subsystems are AMGR, ADXR, BCDB, vgld, CHAN, DISC, PEER, RPCS, SCRP, SRVR, SYNC, and TXMP.\n" +
	...
	"decodescriptresult-addresses": "The Vigil addresses associated with this script",
	...
	"getcurrentnet--synopsis": "Get Vigil network the server is running on.",
	...
	"infochainresult-relayfee":        "The minimum relay fee for transactions in VGL/KB",
	...
	"getrawmempoolverboseresult-fee":              "Transaction fee in Vigil",
	...
	"sendrawtransaction-allowhighfees": "Whether or not to allow insanely high fees (vgld does not yet implement this parameter, so it has no effect)",
	...
	// StopCmd help.
	"stop--synopsis": "Shutdown vgld.",
	"stop--result0":  "The string 'vgld stopping.'",
	...
	"validateaddresschainresult-address": "The Vigil address (only when isvalid is true)",

	// ValidateAddressCmd help.
	"validateaddress--synopsis": "Verify an address is valid.",
	"validateaddress-address":   "Vigil address to validate",
	...
	"verifymessage-address":   "The Vigil address to use for the signature",
	...
	// EstimateFee help.
	"estimatefee--synopsis": "Returns the estimated fee in VGL/kb.",
	"estimatefee-numblocks": "(unused)",
	"estimatefee--result0":  "Estimated fee.",

	// EstimateSmartFee help.
	"estimatesmartfee--synopsis":     "Returns the estimated fee using the historical fee data in VGL/kb.",
	...
	"estimatesmartfeeresult-feerate": "The Estimated fee rate (in VGL/KB).",
	...
	// TicketFeeInfo help.
	"ticketfeeinfo--synopsis":            "Get various information about ticket fees from the mempool, blocks, and difficulty windows (units: VGL/kB)",
	...
	"ticketfeeinforesult-feeinfomempool": "Ticket fee information for all tickets in the mempool (units: VGL/kB)",
	"ticketfeeinforesult-feeinfoblocks":  "Ticket fee information for a given list of blocks descending from the chain tip (units: VGL/kB)",
	"ticketfeeinforesult-feeinfowindows": "Ticket fee information for a window period where the stake difficulty was the same (units: VGL/kB)",
...
func (c *helpCacher) RPCMethodHelp(method types.Method) (string, error) {
	...
	help, err := VGLjson.GenerateHelp(method, helpDescsEnUS, resultTypes...)
	if err != nil {
		return "", err
	}
...
func (c *helpCacher) RPCUsage(includeWebsockets bool) (string, error) {
	...
	for k := range rpcHandlers {
		usage, err := VGLjson.MethodUsageText(k)
		if err != nil {
			return "", err
		}
