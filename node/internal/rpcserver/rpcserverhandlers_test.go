...
type testRPCChain struct {
	...
	blockByHash                   *VGLutil.Block
	blockByHashErr                error
	blockByHeight                 *VGLutil.Block
	...
	ticketPoolValue               VGLutil.Amount
...
func (c *testRPCChain) BlockByHash(hash *chainhash.Hash) (*VGLutil.Block, error) {
	return c.blockByHash, c.blockByHashErr
}

// BlockByHeight returns a mocked block at the given height.
func (c *testRPCChain) BlockByHeight(height int64) (*VGLutil.Block, error) {
...
// ticket pool.
func (c *testRPCChain) TicketPoolValue() (VGLutil.Amount, error) {
...
// to the given block.
func (c *testRPCChain) TSpendCountVotes(*chainhash.Hash, *VGLutil.Tx) (int64, int64, error) {
...
func (c *testRPCChain) IsKawpowAgendaActive(*chainhash.Hash) (bool, error) {
	return c.kawpowActive, c.kawpowActiveErr
}
...
func (s *testSyncManager) SubmitBlock(block *VGLutil.Block) error {
...
func (s *testSyncManager) ProcessTransaction(tx *VGLutil.Tx, allowOrphans bool,
	allowHighFees bool, tag mempool.Tag) ([]*VGLutil.Tx, error) {
...
func (c *testConnManager) RelayTransactions(txns []*VGLutil.Tx) {}
...
func (s *testSanityChecker) CheckBlockSanity(block *VGLutil.Block) error {
...
// transaction pool.
func (mp *testTxMempooler) FetchTransaction(txHash *chainhash.Hash) (*VGLutil.Tx, error) {
...
// for processing.
func (mgr *testNtfnManager) NotifyBlockConnected(block *VGLutil.Block) {}

// NotifyBlockDisconnected passes a block disconnected to the manager
// for processing.
func (mgr *testNtfnManager) NotifyBlockDisconnected(block *VGLutil.Block) {}
...
func (mgr *testNtfnManager) NotifyTSpend(tx *VGLutil.Tx) {}
...
// manager for processing.
func (mgr *testNtfnManager) NotifyMempoolTx(tx *VGLutil.Tx, isNew bool) {}
	...
	testChainParams.Deployments = map[uint32][]chaincfg.ConsensusDeployment{
		7: {{
			Vote: chaincfg.Vote{
				...
				Description: "Enable header commitments as defined in VGLP0005",
...
func defaultMockLogManager() *testLogManager {
	return &testLogManager{
		supportedSubsystems: []string{"vgld", "PEER", "RPCS"},
	}
}
...
func defaultMockConfig(chainParams *chaincfg.Params) *Config {
	return &Config{
		...
		MinRelayTxFee:      VGLutil.Amount(10000),
...
func TestHandleCreateRawSStx(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		name:    "handleCreateRawSStx: invalid amount > VGLutil.MaxAmount",
		handler: handleCreateRawSStx,
		cmd: &types.CreateRawSStxCmd{
			Inputs: defaultCmdInputs,
			Amount: map[string]int64{
				"DcuQKx8BES9wU7C6Q5VmLBjw436r27hayjS": VGLutil.MaxAmount + 1,
			},
			COuts: defaultCmdCOuts,
		},
		wantErr: true,
		...
		wantErr: true,
		errCode: VGLjson.ErrRPCInvalidAddressOrKey,
	}, {
		...
		name:    "handleCreateRawSStx: invalid change amount > VGLutil.MaxAmount",
		handler: handleCreateRawSStx,
		cmd: &types.CreateRawSStxCmd{
			Inputs: []types.SStxInput{{
				Txid: "e02f03a25a57afdd402818fe5b13985a0731502ad8a8c93d1874900e84d3330d",
...
func TestHandleCreateRawSSRtx(t *testing.T) {
	...
	defaultFee := VGLjson.Float64(0.1)
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		wantErr: true,
		errCode: VGLjson.ErrRPCBlockNotFound,
	}, {
		...
		cmd: &types.CreateRawSSRtxCmd{
			Inputs: defaultCmdInputs,
			Fee:    VGLjson.Float64(math.Inf(1)),
		},
		wantErr: true,
		errCode: VGLjson.ErrRPCInvalidParameter,
	}, {
		...
		wantErr: true,
		errCode: VGLjson.ErrRPCNoTxInfo,
	}, {
		...
		wantErr: true,
		errCode: VGLjson.ErrRPCDeserialization,
	}, {
...
func TestHandleCreateRawTransaction(t *testing.T) {
	...
	defaultCmdLockTime := VGLjson.Int64(1)
	defaultCmdExpiry := VGLjson.Int64(1)
	testRPCServerHandler(t, []rpcTest{{
		...
		cmd: &types.CreateRawTransactionCmd{
			Inputs: []types.TransactionInput{{
				Amount: (VGLutil.MaxAmount + 1) / 1e8,
				Txid:   "e02f03a25a57afdd402818fe5b13985a0731502ad8a8c93d1874900e84d3330d",
				Vout:   0,
				Tree:   0,
			}},
			Amounts: map[string]float64{
				"DcurAwesomeAddressmqDctW5wJCW1Cn2MF": (VGLutil.MaxAmount + 1) / 1e8,
			},
			LockTime: defaultCmdLockTime,
			Expiry:   defaultCmdExpiry,
		},
...
func TestHandleEstimateStakeDiff(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		result: &types.EstimateStakeDiffResult{
			Min:      VGLutil.Amount(low).ToCoin(),
			Max:      VGLutil.Amount(high).ToCoin(),
			Expected: VGLutil.Amount(med).ToCoin(),
			User:     VGLjson.Float64(VGLutil.Amount(user).ToCoin()),
		},
	}, {
...
func TestHandleGenerate(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		mockCPUMiner:    cpu,
		wantErr:         true,
		errCode:         VGLjson.ErrRPCDifficulty,
	}, {
...
func TestHandleGetAddedNodeInfo(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		cmd: &types.GetAddedNodeInfoCmd{
			DNS:  false,
			Node: VGLjson.String("127.0.0.211:9108"),
		},
		...
		cmd: &types.GetAddedNodeInfoCmd{
			DNS:  false,
			Node: VGLjson.String("127.0.0.212:9108"),
		},
		wantErr: true,
		errCode: VGLjson.ErrRPCInternal.Code,
	}, {
		...
		cmd: &types.GetAddedNodeInfoCmd{
			DNS:  true,
			Node: VGLjson.String("mydomain.org:9108"),
		},
		result: []*types.GetAddedNodeInfoResult{{
			AddedNode: "mydomain.org:9108",
			Connected: VGLjson.Bool(true),
...
func TestHandleGetBlock(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		result: types.GetBlockVerboseResult{
			...
			SBits:         VGLutil.Amount(blkHeader.SBits).ToCoin(),
...
func TestHandleGetBlockHash(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		wantErr: true,
		errCode: VGLjson.ErrRPCOutOfRange,
	}})
}
...
func TestHandleGetMempoolInfo(t *testing.T) {
	...
	txDescOne := &mempool.TxDesc{
		TxDesc: mining.TxDesc{
			Tx:   VGLutil.NewTx(block432100.Transactions[0]),
	...
	txDescTwo := &mempool.TxDesc{
		TxDesc: mining.TxDesc{
			Tx:   VGLutil.NewTx(block432100.Transactions[1]),
...
func TestHandleGetPeerInfo(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
			...
			connManager.connectedPeers = []Peer{
				&testPeer{
					...
					statsSnapshot: &peer.StatsSnap{
						...
						TimeOffset:     int64(-75),
						Version:        uint32(6),
						UserAgent:      "/VGLwire:0.3.0/vgld:1.5.0(pre)/",
						Inbound:        false,
		...
		result: []*types.GetPeerInfoResult{{
			...
			PingWait:       float64(2),
			Version:        uint32(6),
			SubVer:         "/VGLwire:0.3.0/vgld:1.5.0(pre)/",
			Inbound:        false,
			StartingHeight: int64(323327),
...
func TestHandleGetTreasuryBalance(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		handler: handleGetTreasuryBalance,
		cmd: &types.GetTreasuryBalanceCmd{
			Hash: VGLjson.String("invalid hex"),
		},
		wantErr: true,
		errCode: VGLjson.ErrRPCDecodeHexString,
	}, {
		...
		wantErr: true,
		errCode: VGLjson.ErrRPCNoTreasury,
	}, {
...
func TestHandleGetTxOut(t *testing.T) {
	...
	txOutResultMempool := types.GetTxOutResult{
		...
		Confirmations: 0,
		Value:         VGLutil.Amount(txOut.Value).ToUnit(VGLutil.AmountCoin),
		ScriptPubKey: types.ScriptPubKeyResult{
			Asm:       disbuf,
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		cmd: &types.GetTxOutCmd{
			...
			Vout:           vout,
			Tree:           wire.TxTreeRegular,
			IncludeMempool: VGLjson.Bool(false),
		},
		mockTxMempooler: mempoolerWithTx(),
		mockChain: func() *testRPCChain {
			chain := chainWithTx()
		...
		mockTxMempooler: func() *testTxMempooler {
			mp := defaultMockTxMempooler()
			tx := VGLutil.NewTx(hexToMsgTx(txHex))
		...
		mockTxMempooler: mempoolerWithTx(),
		wantErr:         true,
		errCode:         VGLjson.ErrRPCInvalidTxVout,
	}, {
...
func TestHandleNode(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		cmd: &types.NodeCmd{
			SubCmd:        "connect",
			Target:        "127.0.0.210:9108",
			ConnectSubCmd: VGLjson.String("temp"),
		},
		result: nil,
	}, {
		...
		cmd: &types.NodeCmd{
			SubCmd:        "connect",
			Target:        "127.0.0.210:9108",
			ConnectSubCmd: VGLjson.String("invalid"),
		},
		wantErr: true,
		errCode: VGLjson.ErrRPCInvalidParameter,
	}, {
...
func TestHandleGetWork(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		mockMiningState: defaultMockMiningState(),
		wantErr:         true,
		errCode:         VGLjson.ErrRPCClientNotConnected,
	}, {
		...
		wantErr: true,
		errCode: VGLjson.ErrRPCClientInInitialDownload,
	}, {
...
func TestHandleReconsiderBlock(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		wantErr:   true,
		errCode:   VGLjson.ErrRPCReconsiderFailure,
	}, {
...
func TestHandleTSpendVotes(t *testing.T) {
	...
	*futureBestSnap = *chainFarFuture.bestSnapshot
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		mockChain: func() *testRPCChain {
			chainMinedTSpend.blockByHash = VGLutil.NewBlock(blockMinedTSpend)
		...
		mockTxMempooler: mempoolTxNotFound,
		mockChain: func() *testRPCChain {
			chainFarFuture.blockByHash = VGLutil.NewBlock(blockMinedTSpend)
...
func TestHandleTxFeeInfo(t *testing.T) {
	...
	txDesc.Tx = VGLutil.NewTx(block432100.Transactions[1])
	txDesc.Type = stake.TxTypeRegular
	txDesc.Added = time.Time{}
	fee, _ := VGLutil.NewAmount(0.0030)
...
func TestHandleVerifyMessage(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		wantErr: true,
		errCode: VGLjson.ErrRPCParse.Code,
	}, {
...
func TestHandleSendRawTransaction(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		wantErr: true,
		errCode: VGLjson.ErrRPCDuplicateTx,
	}, {
		...
		mockSyncManager: func() *testSyncManager {
			syncManager := defaultMockSyncManager()
			syncManager.processTransaction = []*VGLutil.Tx{tx}
			return syncManager
		}(),
...
func TestHandleGetRawMempool(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		handler:         handleGetRawMempool,
		mockTxMempooler: mockTxMempooler,
		cmd:             &types.GetRawMempoolCmd{TxType: VGLjson.String("regular")},
		result:          []string{regularHash},
	}, {
		name:            "handleGetRawMempool: ok ticket",
		handler:         handleGetRawMempool,
		mockTxMempooler: mockTxMempooler,
		cmd:             &types.GetRawMempoolCmd{TxType: VGLjson.String("tickets")},
		result:          []string{ticketHash},
	}, {
		name:            "handleGetRawMempool: ok vote",
		handler:         handleGetRawMempool,
		mockTxMempooler: mockTxMempooler,
		cmd:             &types.GetRawMempoolCmd{TxType: VGLjson.String("votes")},
		result:          []string{voteHash},
	}, {
		name:            "handleGetRawMempool: ok revocation",
		handler:         handleGetRawMempool,
		mockTxMempooler: mockTxMempooler,
		cmd:             &types.GetRawMempoolCmd{TxType: VGLjson.String("revocations")},
		result:          []string{revocationHash},
	}, {
		name:            "handleGetRawMempool: ok tSpend",
		handler:         handleGetRawMempool,
		mockTxMempooler: mockTxMempooler,
		cmd:             &types.GetRawMempoolCmd{TxType: VGLjson.String("tspend")},
		result:          []string{tSpendHash},
	}, {
		name:            "handleGetRawMempool: ok tAdd",
		handler:         handleGetRawMempool,
		mockTxMempooler: mockTxMempooler,
		cmd:             &types.GetRawMempoolCmd{TxType: VGLjson.String("tadd")},
		...
		handler:         handleGetRawMempool,
		mockTxMempooler: mockTxMempooler,
		cmd:             &types.GetRawMempoolCmd{TxType: VGLjson.String("not a type")},
		wantErr:         true,
		errCode:         VGLjson.ErrRPCInvalidParameter,
	}})
}
...
func TestHandleGetRawTransaction(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		mockTxMempooler: func() *testTxMempooler {
			mp := defaultMockTxMempooler()
			mp.fetchTransaction = VGLutil.NewTx(&tx)
			mp.fetchTransactionErr = nil
			return mp
		}(),
		mockDB: func() *testDB {
...
func TestHandleStartProfiler(t *testing.T) {
	...
	testRPCServerHandler(t, []rpcTest{{
		...
		cmd:     &types.StartProfilerCmd{Addr: "127.0.0.1:6060"},
		wantErr: true,
		errCode: VGLjson.ErrRPCProfilerState,
	}, {
