...
type ConnManager interface {
	...
	RelayTransactions(txns []*VGLutil.Tx)
...
type SyncManager interface {
	...
	SubmitBlock(block *VGLutil.Block) error
	...
	ProcessTransaction(tx *VGLutil.Tx, allowOrphans bool, allowHighFees bool,
		tag mempool.Tag) ([]*VGLutil.Tx, error)
...
type Chain interface {
	...
	BlockByHash(hash *chainhash.Hash) (*VGLutil.Block, error)

	// BlockByHeight returns the block at the given height in the main chain.
	BlockByHeight(height int64) (*VGLutil.Block, error)
	...
	// revocations agenda vote, as defined in VGLP0009, has passed and is now
	...
	// split agenda vote, as defined in VGLP0010, has passed and is now active
	...
	// split round 2 agenda vote, as defined in VGLP0012, has passed and is now
	// active for the block AFTER the given block.
	...
	// the specified block.
	TSpendCountVotes(*chainhash.Hash, *VGLutil.Tx) (int64, int64, error)
...
type FeeEstimator interface {
	...
	EstimateFee(targetConfs int32) (VGLutil.Amount, error)
}
...
type SanityChecker interface {
	...
	// per consensus.
	CheckBlockSanity(block *VGLutil.Block) error
}
...
type TxMempooler interface {
	...
	FetchTransaction(txHash *chainhash.Hash) (*VGLutil.Tx, error)
...
type NtfnManager interface {
	...
	// for processing.
	NotifyBlockConnected(block *VGLutil.Block)

	// NotifyBlockDisconnected passes a block disconnected to the manager
	// for processing.
	NotifyBlockDisconnected(block *VGLutil.Block)
	...
	// manager for processing.
	NotifyMempoolTx(tx *VGLutil.Tx, isNew bool)
