...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *VGLutil.Block) {
	select {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *VGLutil.Block) {
	select {
...
func (m *wsNotificationManager) NotifyTSpend(tx *VGLutil.Tx) {
	select {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *VGLutil.Tx, isNew bool) {
...
// Notification types.
type notificationBlockConnected VGLutil.Block
type notificationBlockDisconnected VGLutil.Block
type notificationWork mining.TemplateNtfn
type notificationTSpend VGLutil.Tx
	...
	for {
		...
		case n, ok := <-m.notificationMsgs:
			...
			case *notificationBlockConnected:
				m.notifyBlockConnected(blockNotifications, (*VGLutil.Block)(n))
			...
			case *notificationTSpend:
				m.notifyTSpend(tspendNotifications, (*VGLutil.Tx)(n))
...
func (m *wsNotificationManager) subscribedClients(tx *VGLutil.Tx, clients map[chan struct{}]*wsClient) map[chan struct{}]struct{} {
...
func (m *wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient, block *VGLutil.Block) {
	...
	for quitChan, client := range clients {
		...
		marshalledJSON, err := VGLjson.MarshalCmd("1.0", nil, &ntfn)
		if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *VGLutil.Block) {
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *VGLutil.Tx) {
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			marshalledJSONVerbose, err = VGLjson.MarshalCmd("1.0", nil,
				verboseNtfn)
			if err != nil {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *VGLutil.Tx,
	...
	if len(clientsToNotify) != 0 {
		...
		marshalled, err := VGLjson.MarshalCmd("1.0", nil, n)
		if err != nil {
...
func (m *wsNotificationManager) notifyMixMessage(clients map[chan struct{}]*wsClient,
	...
	marshaledJSON, err := VGLjson.MarshalCmd("1.0", nil, ntfn)
	if err != nil {
	...
	for !c.disconnected.Load() {
		...
		if !batchedRequest {
			...
			if !c.isAdmin {
				if _, ok := rpcLimited[req.Method]; !ok {
					jsonErr := &VGLjson.RPCError{
						Code:    VGLjson.ErrRPCInvalidParams.Code,
		...
		if batchedRequest {
			...
			if err != nil {
				...
				reply, err = VGLjson.MarshalResponse("2.0", nil, nil, jsonErr)
				if err != nil {
...
func handleWebsocketHelp(_ context.Context, wsc *wsClient, icmd interface{}) (interface{}, error) {
	...
	if !ok {
		return nil, VGLjson.ErrRPCInternal
	}
...
// string slice.
func rescanBlock(filter *wsClientFilter, block *VGLutil.Block, params *chaincfg.Params, isTreasuryEnabled bool) []string {
