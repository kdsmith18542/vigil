...
import (
	...
	"github.com/vigilnetwork/vgl/internal/mempool"
	"github.com/vigilnetwork/vgl/internal/mining"
...
const (
	...
	// plus the mix hash (32 bytes) and 64-bit nonce (8 bytes).
	getworkDataLenKawPow = wire.MaxBlockHeaderPayload + 32 + 8
...
var (
	...
	ErrRPCUnimplemented = &VGLjson.RPCError{
	...
	ErrRPCNoWallet = &VGLjson.RPCError{
		Code:    VGLjson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which vgld has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of vglwallet.
...
// needed.
func rpcInternalErr(err error, context string) *VGLjson.RPCError {
	errStr := err.Error()
	logStr := errStr
	if context != "" {
	...
	log.Error(logStr)
	return VGLjson.NewRPCError(VGLjson.ErrRPCInternal.Code, errStr)
}
...
func rpcInvalidError(fmtStr string, args ...interface{}) *VGLjson.RPCError {
	return VGLjson.NewRPCError(VGLjson.ErrRPCInvalidParameter,
...
func rpcDeserializationError(fmtStr string, args ...interface{}) *VGLjson.RPCError {
	return VGLjson.NewRPCError(VGLjson.ErrRPCDeserialization,
		fmt.Sprintf(fmtStr, args...))
}

// rpcRuleError is a convenience function to convert a
// rule error to an RPC error with the appropriate code set.
func rpcRuleError(fmtStr string, args ...interface{}) *VGLjson.RPCError {
	return VGLjson.NewRPCError(VGLjson.ErrRPCMisc,
		fmt.Sprintf(fmtStr, args...))
}
...
func rpcDuplicateTxError(fmtStr string, args ...interface{}) *VGLjson.RPCError {
	return VGLjson.NewRPCError(VGLjson.ErrRPCDuplicateTx,
		fmt.Sprintf(fmtStr, args...))
}
...
// not needed.
func rpcAddressKeyError(fmtStr string, args ...interface{}) *VGLjson.RPCError {
	return VGLjson.NewRPCError(VGLjson.ErrRPCInvalidAddressOrKey,
		fmt.Sprintf(fmtStr, args...))
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *VGLjson.RPCError {
	return VGLjson.NewRPCError(VGLjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *VGLjson.RPCError {
	return VGLjson.NewRPCError(VGLjson.ErrRPCNoTxInfo,
...
// the blockchain.
func rpcBlockNotFoundError(blockHash chainhash.Hash) *VGLjson.RPCError {
	return VGLjson.NewRPCError(VGLjson.ErrRPCBlockNotFound,
...
// the mixpool.
func rpcMixMessageNotFoundError(hash *chainhash.Hash) *VGLjson.RPCError {
	return VGLjson.NewRPCError(VGLjson.ErrRPCNoMixMsgInfo,
...
func rpcConnectionClosedError() *VGLjson.RPCError {
	return VGLjson.NewRPCError(VGLjson.ErrRPCMisc, "Connection closed")
}
...
func rpcMiscError(message string) *VGLjson.RPCError {
	return VGLjson.NewRPCError(VGLjson.ErrRPCMisc, message)
}
...
func handleCreateRawSStx(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	for encodedAddr, amount := range c.Amount {
		// Ensure amount is in the valid range for monetary amounts.
		if amount <= 0 || amount > VGLutil.MaxAmount {
	...
	for i, cout := range c.COuts {
		...
		// Ensure amount is in the valid range for monetary amounts.
		if cout.ChangeAmt < 0 || cout.ChangeAmt > VGLutil.MaxAmount {
			return nil, rpcInvalidError("Invalid change amount: 0 "+
				"> %v > %v", cout.ChangeAmt, VGLutil.MaxAmount)
		}
...
func handleCreateRawSSRtx(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	ticketSubmissionAmount := VGLutil.Amount(ticketSubmission.Value)
	inputAmount, err := VGLutil.NewAmount(input.Amount)
	if err != nil {
		return nil, rpcInvalidError("%v", err)
	}
	...
	var feeAmt VGLutil.Amount
	if c.Fee != nil {
		var err error
		feeAmt, err = VGLutil.NewAmount(*c.Fee)
		if err != nil {
...
func handleEstimateStakeDiff(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	if c.Tickets != nil {
		...
		userEstFlt := VGLutil.Amount(userEst).ToCoin()
		userEstFltPtr = &userEstFlt
	}

	return &types.EstimateStakeDiffResult{
		Min:      VGLutil.Amount(min).ToCoin(),
		Max:      VGLutil.Amount(max).ToCoin(),
		Expected: VGLutil.Amount(expected).ToCoin(),
		User:     userEstFltPtr,
	}, nil
}
...
func handleGetAddedNodeInfo(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	// Retrieve a list of persistent (added) peers from the Vigil server
	...
	for _, peer := range peers {
		...
		result.Connected = VGLjson.Bool(peer.Connected())
...
func handleGetBlock(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	if err != nil {
		return nil, &VGLjson.RPCError{
			Code:    VGLjson.ErrRPCBlockNotFound,
...
func handleGetBlockSubsidy(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	case isSubsidyEnabled:
		subsidySplitVariant = standalone.SSVVGLP0010
	}
	...
	if c.Verbose != nil && *c.Verbose {
		...
		for i := range descs {
			...
			mpd := &types.GetRawMempoolVerboseResult{
				...
				Fee:              VGLutil.Amount(desc.Fee).ToCoin(),
...
func handleGetStakeDifficulty(_ context.Context, s *Server, _ interface{}) (interface{}, error) {
	...
	result := types.GetStakeDifficultyResult{
		CurrentStakeDifficulty: VGLutil.Amount(blockHeader.SBits).ToCoin(),
		NextStakeDifficulty:    VGLutil.Amount(best.NextStakeDiff).ToCoin(),
	}
	return result, nil
}
...
func handleGetTreasurySpendVotes(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	if c.TSpends != nil && len(*c.TSpends) > 0 {
		...
		tspends = make([]*VGLutil.Tx, len(*c.TSpends))
		...
		tspends = make([]*VGLutil.Tx, len(hashes))
...
func handleGetTxOut(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	if txFromMempool != nil {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &VGLjson.RPCError{
				Code: VGLjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &types.GetTxOutResult{
		...
		Confirmations: confirmations,
		Value:         VGLutil.Amount(value).ToUnit(VGLutil.AmountCoin),
...
func getWorkTemplateKey(header *wire.BlockHeader) [merkleRootPairSize]byte {
	...
	// Note that with VGLP0005 active, the stake root field will be merged with
...
func handleGetWorkRequest(ctx context.Context, s *Server) (interface{}, error) {
	...
	// Determine if the current best block is a KawPoW block.
	// The isKawpowAgendaActive check is no longer needed here as CurrentTemplate handles it.
	// The template and err variables are declared and assigned below.
...
func handleGetWorkSubmission(_ context.Context, s *Server, hexData string) (interface{}, error) {
	...
	// For KawPoW blocks, extract the mix hash and nonce from the work data
	if submittedHeader.Version >= 3 { // Assuming version 3+ indicates KawPoW blocks
		// Ensure we have enough data for mix hash (32 bytes) and nonce (8 bytes)
		if len(data) < wire.MaxBlockHeaderPayload+32+8 {
			return false, rpcInvalidError("Insufficient data for KawPoW mix hash and nonce")
		}

		// Extract mix hash (32 bytes after the header)
		var mixHash [32]byte
		copy(mixHash[:], data[wire.MaxBlockHeaderPayload:wire.MaxBlockHeaderPayload+32])
		submittedHeader.MixHash = mixHash

		// Extract nonce (8 bytes after mix hash)
		submittedHeader.Nonce = binary.LittleEndian.Uint64(data[wire.MaxBlockHeaderPayload+32 : wire.MaxBlockHeaderPayload+40])
	}
	...
	isKawpowActive, err := s.isKawpowAgendaActive(prevBlkHash)
	if err != nil {
	...
	if isKawpowActive {
		// Verify the KawPoW solution using the mix hash and nonce
		// Convert the header hash to a chainhash.Hash for verification
		...
		err = standalone.CheckProofOfWork(&finalHash, submittedHeader.Bits,
		...
		if err != nil {
			...
			if !errors.As(err, &rErr) {
				const context = "Unexpected error while checking KawPoW proof of work"
				return false, rpcInternalErr(err, context)
			}
			
			log.Errorf("Block submitted via getwork does not meet the required "+
				"KawPoW proof of work: header %s, nonce %d, mixhash %s",
				headerHash, submittedHeader.Nonce, submittedHeader.MixHash)
			return false, nil
		}
		
		// Verify that the mix hash matches what we calculated
		if mixHash != submittedHeader.MixHash {
			log.Errorf("Block submitted via getwork has invalid mix hash: "+
	...
	block := VGLutil.NewBlock(&msgBlock)
...
func handleGetWork(ctx context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	if !s.cfg.AllowUnsyncedMining && s.cfg.ConnMgr.ConnectedCount() == 0 {
		return nil, &VGLjson.RPCError{
			...
			Message: "Vigil is not connected",
		}
	}
	...
	if !s.cfg.AllowUnsyncedMining && !initialChainState && !chain.IsCurrent() {
		return nil, &VGLjson.RPCError{
			Code:    VGLjson.ErrRPCClientInInitialDownload,
			Message: "Vigil is downloading blocks...",
		}
	}
...
func handleReconsiderBlock(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	if err != nil {
		...
		if allRuleErrs(err) {
			return nil, &VGLjson.RPCError{
				Code: VGLjson.ErrRPCReconsiderFailure,
...
func handleSendRawMixMessage(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	s.kawpowHasherMu.Lock()
	msg.WriteHash(s.kawpowHasher)
	s.kawpowHasherMu.Unlock()
...
func handleStartProfiler(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	if listeners := s.cfg.ProfilerMgr.Listeners(); len(listeners) > 0 {
		...
		return nil, VGLjson.NewRPCError(VGLjson.ErrRPCProfilerState, str)
	}
...
func handleSubmitBlock(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	block, err := VGLutil.NewBlockFromBytes(serializedBlock)
	if err != nil {
		return nil, rpcInternalErr(err, "Block decode")
	}
...
func min(s []VGLutil.Amount) VGLutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func max(s []VGLutil.Amount) VGLutil.Amount {
	max := VGLutil.Amount(0)
	for i := range s {
		if s[i] > max {
			max = s[i]
		}
...
func mean(s []VGLutil.Amount) VGLutil.Amount {
	sum := VGLutil.Amount(0)
	for i := range s {
		sum += s[i]
	}
	...
	return sum / VGLutil.Amount(len(s))
}

// median gets the median amount from a slice of amounts.
func median(s []VGLutil.Amount) VGLutil.Amount {
	if len(s) == 0 {
		return 0
	}

	sort.Sort(VGLutil.AmountSorter(s))
...
func stdDev(s []VGLutil.Amount) VGLutil.Amount {
	var total float64
	mean := mean(s)
	...
	amt, _ := VGLutil.NewAmount(math.Sqrt(v))
...
func feeInfoForMempool(s *Server, txType stake.TxType) *types.FeeInfoMempool {
	...
	ticketFees := make([]VGLutil.Amount, 0, len(txDs))
	for _, txD := range txDs {
		if txD.Type == txType {
			feePerKb := (VGLutil.Amount(txD.Fee)) * 1000 /
				VGLutil.Amount(txD.Tx.MsgTx().SerializeSize())
...
// properly set.
func calcFeePerKb(tx *VGLutil.Tx) VGLutil.Amount {
	var in VGLutil.Amount
	for _, txIn := range tx.MsgTx().TxIn {
		in += VGLutil.Amount(txIn.ValueIn)
	}
	var out VGLutil.Amount
	for _, txOut := range tx.MsgTx().TxOut {
		out += VGLutil.Amount(txOut.Value)
	}

	return ((in - out) * 1000) / VGLutil.Amount(tx.MsgTx().SerializeSize())
}
...
func ticketFeeInfoForBlock(s *Server, height int64, txType stake.TxType) (*types.FeeInfoBlock, error) {
	...
	txFees := make([]VGLutil.Amount, txNum)
	itr := 0
...
func handleTicketVWAP(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	return VGLutil.Amount(vwap).ToCoin(), nil
}
...
func handleVerifyMessage(_ context.Context, s *Server, cmd interface{}) (interface{}, error) {
	...
	var buf bytes.Buffer
	wire.WriteVarString(&buf, 0, "Vigil Signed Message:\n")
...
// provided block hash.
func (s *Server) isKawpowAgendaActive(prevBlkHash *chainhash.Hash) (bool, error) {
	chain := s.cfg.Chain
...
func (s *Server) NotifyNewTransactions(txns []*VGLutil.Tx) {
	for _, tx := range txns {
...
func (s *Server) NotifyTSpend(tx *VGLutil.Tx) {
...
func (s *Server) NotifyBlockConnected(block *VGLutil.Block) {
...
func (s *Server) NotifyBlockDisconnected(block *VGLutil.Block) {
...
func (s *Server) standardCmdResult(ctx context.Context, cmd *parsedRPCCmd) (interface{}, error) {
	...
	if !ok {
		return nil, VGLjson.ErrRPCMethodNotFound
	}
...
func parseCmd(request *VGLjson.Request) *parsedRPCCmd {
	...
	params, err := VGLjson.ParseParams(method, request.Params)
	if err != nil {
		...
		if errors.Is(err, VGLjson.ErrUnregisteredMethod) {
			parsedCmd.err = VGLjson.ErrRPCMethodNotFound
...
// type *VGLjson.RPCError to the appropriate type as needed.
func createMarshalledReply(rpcVersion string, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	...
	return VGLjson.MarshalResponse(rpcVersion, id, result, jsonErr)
}

// processRequest determines the incoming request type (single or batched),
// parses it and returns a marshalled response.
func (s *Server) processRequest(ctx context.Context, request *VGLjson.Request, isAdmin bool) []byte {
	...
	if jsonErr == nil {
		if request.Method == "" {
			jsonErr = &VGLjson.RPCError{
				Code:    VGLjson.ErrRPCInvalidRequest.Code,
...
func (s *Server) jsonRPCRead(sCtx context.Context, w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err != nil {
			...
			resp, err = VGLjson.MarshalResponse("1.0", nil, nil, jsonErr)
			if err != nil {
	...
	if batchedRequest {
		...
		if err != nil {
			...
			resp, err = VGLjson.MarshalResponse("2.0", nil, nil, jsonErr)
			if err != nil {
				log.Errorf("Failed to create reply: %v", err)
			}
			...
			if len(batchedRequests) > 0 {
				...
				for _, entry := range batchedRequests {
					...
					if err != nil {
						...
						resp, err = VGLjson.MarshalResponse("", nil, nil, jsonErr)
						if err != nil {
							log.Errorf("Failed to create reply: %v", err)
						}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="vgld RPC"`)
...
func New(config *Config) (*Server, error) {
	rpc := Server{
		...
		kawpowHasher:         kawpow.New(), // Assuming kawpow.New() is a constructor or accessible function
	// TODO: Ensure kawpow package is properly imported and its components are accessible.
	// If kawpow.New() is not directly accessible, it might need to be defined or imported from a specific path.
	// For now, assuming it's a constructor or globally available after import.
	}
	key := make([]byte, 32)
	rand.Read(key)
