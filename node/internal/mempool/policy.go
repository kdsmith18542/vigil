...
const (
	...
	DefaultMinRelayTxFee = VGLutil.Amount(1e4)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee VGLutil.Amount) int64 {
	...
	if minFee < 0 || minFee > VGLutil.MaxAmount {
		minFee = VGLutil.MaxAmount
	}
...
// instance.
func checkInputsStandard(tx *VGLutil.Tx, txType stake.TxType, utxoView *blockchain.UtxoViewpoint, isTreasuryEnabled bool) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee VGLutil.Amount) bool {
...
// Note: all non-nil errors MUST be RuleError with an underlying TxRuleError
// instance.
func checkTransactionStandard(tx *VGLutil.Tx, txType stake.TxType, height int64,
	medianTime time.Time, minRelayTxFee VGLutil.Amount) error {
