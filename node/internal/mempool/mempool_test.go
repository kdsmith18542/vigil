...
type fakeChain struct {
	...
	blocks        map[chainhash.Hash]*VGLutil.Block
...
func (s *fakeChain) FetchUtxoView(tx *VGLutil.Tx, treeValid bool) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) BlockByHash(hash *chainhash.Hash) (*VGLutil.Block, error) {
	s.RLock()
...
func (s *fakeChain) AddBlock(block *VGLutil.Block) {
	s.Lock()
...
func (s *fakeChain) CalcSequenceLock(tx *VGLutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
...
func (s *fakeChain) AddFakeUtxoMedianTime(tx *VGLutil.Tx, txOutIdx uint32, medianTime time.Time) {
	s.Lock()
...
// transactions.
func txOutToSpendableOut(tx *VGLutil.Tx, outputNum uint32, tree int8) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum, Tree: tree},
		amount:   VGLutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
func (p *poolHarness) GetKey(addr stdaddr.Address) ([]byte, VGLec.SignatureType, bool, error) {
	return p.signKey, p.sigType, true, nil
}

// AddFakeUTXO creates a fake mined utxo for the provided transaction.
func (p *poolHarness) AddFakeUTXO(tx *VGLutil.Tx, blockHeight int64, blockIndex uint32) {
...
func (p *poolHarness) determineSubsidySplitVariant() standalone.SubsidySplitVariant {
	...
	case p.subsidySplitR2Active:
		return standalone.SSVVGLP0012
	case p.subsidySplitActive:
		return standalone.SSVVGLP0010
	}
...
func (p *poolHarness) CreateCoinbaseTx(blockHeight int64, numOutputs uint32) (*VGLutil.Tx, error) {
...
// testing.
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput, numOutputs uint32, mungers ...func(*wire.MsgTx)) (*VGLutil.Tx, error) {
	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput VGLutil.Amount
	...
	if totalFee > int64(totalInput) {
		return nil, fmt.Errorf("unable to create signed transaction: required "+
			"fee %s > input amount %s", VGLutil.Amount(totalFee), totalInput)
	}
	...
	for i := range tx.TxIn {
		sigScript, err := sign.SignatureScript(tx, i, p.payScript,
			txscript.SigHashAll, p.signKey, VGLec.STEcdsaSecp256k1, true)
		if err != nil {
			return nil, err
		}
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*VGLutil.Tx, error) {
	txChain := make([]*VGLutil.Tx, 0, numTxns)
...
// output.
func (p *poolHarness) CreateTx(out spendableOutput) (*VGLutil.Tx, error) {
...
// output.
func (p *poolHarness) CreateTicketPurchase(input spendableOutput, cost int64, mungers ...func(*wire.MsgTx)) (*VGLutil.Tx, error) {
...
func (p *poolHarness) CreateTicketPurchaseFromTx(sourceTx *VGLutil.Tx, cost int64) (*VGLutil.Tx, error) {
...
// the opportunity to modify the transaction which is especially useful for
// testing.
func (p *poolHarness) CreateVote(ticket *VGLutil.Tx, mungers ...func(*wire.MsgTx)) (*VGLutil.Tx, error) {
	...
	return VGLutil.NewTx(vote), nil
}

// CreateRevocation creates a revocation using the provided ticket.
func (p *poolHarness) CreateRevocation(ticket *VGLutil.Tx,
	revocationTxFee VGLutil.Amount) (*VGLutil.Tx, error) {
	...
	return VGLutil.NewTx(revocation), nil
}
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	chain := &fakeChain{
		...
		blocks:      make(map[chainhash.Hash]*VGLutil.Block),
	...
	harness = &poolHarness{
		signKey:      keyBytes,
		sigType:      VGLec.STEcdsaSecp256k1,
		payAddr:      payAddr,
	...
	mockBestBlock := *VGLutil.NewBlock(&wire.MsgBlock{})
...
func testPoolMembership(tc *testContext, tx *VGLutil.Tx, inOrphanPool, inTxPool bool) {
	txHash := tx.Hash()
...
func TestExpirationPruning(t *testing.T) {
	...
	expiringTxns := make([]*VGLutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   VGLutil.Amount(5000000000),
...
func TestSequenceLockAcceptance(t *testing.T) {
	...
	for i, test := range tests {
		...
		inputMsgTx.AddTxOut(newTxOut(1000000000+int64(i), harness.payScriptVer,
			harness.payScript))
		inputTx := VGLutil.NewTx(inputMsgTx)
...
func TestHandlesTSpends(t *testing.T) {
	...
	// correct tspends.
	assertTSpendHashes := func(tspends []*VGLutil.Tx) {
		t.Helper()
	...
	// mempool.
	acceptTSpend := func(tx *VGLutil.Tx) {
		t.Helper()
		_, err = harness.txPool.ProcessTransaction(tx, true, true, 0)
		if err != nil {
	...
	rejectTSpend := func(tx *VGLutil.Tx, wantErr error) {
		t.Helper()
		_, err = harness.txPool.ProcessTransaction(tx, true, true, 0)
	...
	tspends := make([]*VGLutil.Tx, 0, nbTSpends)
	for i := 0; i < nbTSpends; i++ {
		msgTx := createTSpend(t, expiry, tspendAmount, tspendFee, piKey)
		tspends = append(tspends, VGLutil.NewTx(msgTx))
	}
	...
	harness.txPool.cfg.OnTSpendReceived = func(tx *VGLutil.Tx) {
...
func createTAdd(t *testing.T, spend *spendableOutput, payScript, signKey []byte,
	amount, fee VGLutil.Amount, changeAddr stdaddr.StakeAddress) *wire.MsgTx {
	t.Helper()
	...
	tx.TxIn[0].SignatureScript, err = sign.SignatureScript(tx, 0, payScript,
		txscript.SigHashAll, signKey, VGLec.STEcdsaSecp256k1, true)
	if err != nil {
...
func TestHandlesTAdds(t *testing.T) {
	...
	// repeated parameters.
	createTAdd := func(spend *spendableOutput, amount, fee VGLutil.Amount,
		changeAddr stdaddr.StakeAddress) *VGLutil.Tx {
		t.Helper()
		return VGLutil.NewTx(createTAdd(t, spend, harness.payScript,
...
func TestRejectTreasurybases(t *testing.T) {
	...
	// block height.
	createTreasurybase := func(blockHeight int64) (*VGLutil.Tx, error) {
...
func TestExplicitVersionSemantics(t *testing.T) {
	...
	mockInputTx := VGLutil.NewTx(mockInputMsgTx)
...
func TestSubsidySplitSemantics(t *testing.T) {
	...
	harness.chain.SetHeight(harness.chainParams.StakeValidationHeight)
	harness.chain.blocks[hash] = mockBlock
	preVGLP0010Vote, err := harness.CreateVote(ticket)
	if err != nil {
		t.Fatalf("unable to create vote: %v", err)
	}
	...
	postVGLP0010Vote, err := harness.CreateVote(ticket)
	if err != nil {
		t.Fatalf("unable to create vote: %v", err)
	}
	...
	// pool, not in the transaction pool, and not reported as available.
	harness.subsidySplitActive = false
	_, err = txPool.ProcessTransaction(postVGLP0010Vote, false, true, 0)
	if !errors.Is(err, blockchain.ErrBadStakebaseAmountIn) {
		t.Fatal("did not get expected ErrBadStakebaseAmountIn error")
	}
	testPoolMembership(tc, postVGLP0010Vote, false, false)
	...
	// pool, is in the transaction pool, and is reported as available.
	_, err = txPool.ProcessTransaction(preVGLP0010Vote, false, true, 0)
	if err != nil {
		t.Fatalf("failed to accept valid vote %v", err)
	}
	testPoolMembership(tc, preVGLP0010Vote, false, true)
	...
	harness.txPool.RemoveTransaction(preVGLP0010Vote, true)
	testPoolMembership(tc, preVGLP0010Vote, false, false)
	...
	testPoolMembership(tc, postVGLP0010Vote, false, true)
}
...
func TestSubsidySplitR2Semantics(t *testing.T) {
	...
	harness.chain.SetHeight(harness.chainParams.StakeValidationHeight)
	harness.chain.blocks[hash] = mockBlock
	preVGLP0012Vote, err := harness.CreateVote(ticket)
	if err != nil {
		t.Fatalf("unable to create vote: %v", err)
	}
	...
	postVGLP0012Vote, err := harness.CreateVote(ticket)
	if err != nil {
		t.Fatalf("unable to create vote: %v", err)
	}
	...
	// pool, not in the transaction pool, and not reported as available.
	harness.subsidySplitR2Active = false
	_, err = txPool.ProcessTransaction(postVGLP0012Vote, false, true, 0)
	if !errors.Is(err, blockchain.ErrBadStakebaseAmountIn) {
		t.Fatal("did not get expected ErrBadStakebaseAmountIn error")
	}
	testPoolMembership(tc, postVGLP0012Vote, false, false)

	// Attempt to add the vote with the original subsidy when the agenda is NOT
	// active and ensure it is accepted.  Also, ensure it is not in the orphan
	// pool, is in the transaction pool, and is reported as available.
	_, err = txPool.ProcessTransaction(preVGLP0012Vote, false, true, 0)
	if err != nil {
		t.Fatalf("failed to accept valid vote %v", err)
	}
	testPoolMembership(tc, preVGLP0012Vote, false, true)

	// Remove the vote from the pool and ensure it is not in the orphan pool,
	// not in the transaction pool, and not reported as available.
	harness.txPool.RemoveTransaction(preVGLP0012Vote, true)
	testPoolMembership(tc, preVGLP0012Vote, false, false)
	...
	testPoolMembership(tc, postVGLP0012Vote, false, true)
}
...
func TestMaybeAcceptTransactions(t *testing.T) {
	...
	calcFee := func(utilTx *VGLutil.Tx) int64 {
	...
	testExpectedAncestorFee := func(tx *VGLutil.Tx, expectedFee int64) {
		txHash := tx.Hash()
