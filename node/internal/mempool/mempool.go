...
const (
	...
	// VGL/kB, this results in a maximum allowed high fee of 1 VGL/kB.
...
type Config struct {
	...
	FetchUtxoView func(*VGLutil.Tx, bool) (*blockchain.UtxoViewpoint, error)

	// BlockByHash defines the function use to fetch the block identified
	// by the given hash.
	BlockByHash func(*chainhash.Hash) (*VGLutil.Block, error)
	...
	// utxo view.
	CalcSequenceLock func(*VGLutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
	...
	// vote in the mempool.
	OnVoteReceived func(voteTx *VGLutil.Tx)
	...
	OnTSpendReceived func(voteTx *VGLutil.Tx)
...
type Policy struct {
	...
	// MinRelayTxFee defines the minimum transaction fee in VGL/kB to be
...
type TxPool struct {
	...
	transient map[chainhash.Hash]*VGLutil.Tx
...
func (mp *TxPool) insertVote(ssgen *VGLutil.Tx) {
...
// TODO Pruning of the votes map Vigil
...
func (mp *TxPool) removeOrphan(tx *VGLutil.Tx, removeRedeemers bool) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveOrphan(tx *VGLutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *VGLutil.Tx, tag Tag) {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *VGLutil.Tx, tag Tag) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *VGLutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeStagedTransaction(stagedTx *VGLutil.Tx) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) hasMempoolInput(tx *VGLutil.Tx) bool {
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func forEachRedeemer(tx *VGLutil.Tx, outpoints map[wire.OutPoint]*TxDesc,
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) forEachRedeemer(tx *VGLutil.Tx, f func(redeemer *TxDesc)) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) forEachStagedRedeemer(tx *VGLutil.Tx, f func(redeemer *TxDesc)) {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *VGLutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *VGLutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *VGLutil.Tx, txType stake.TxType, isTreasuryEnabled bool) error {
...
func (mp *TxPool) checkVoteDoubleSpend(vote *VGLutil.Tx) error {
	voteTx := vote.MsgTx()
	...
	for _, existingVote := range mp.votes[hashVotedOn] {
		if existingVote.TicketHash == ticketSpent {
			...
			//
			// TODO(Vigil): Ideally the votes map and mempool would be kept in
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *VGLutil.Tx, isTreasuryEnabled bool) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*VGLutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
func (mp *TxPool) newTxDesc(utxoView *blockchain.UtxoViewpoint, tx *VGLutil.Tx,
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptDependents(tx *VGLutil.Tx, isTreasuryEnabled bool) []*VGLutil.Tx {
	mp.mtx.Lock()
	defer mp.mtx.Unlock()
...
// It should also set the VGLutil tree type for the tx as well.
func (mp *TxPool) maybeAcceptTransaction(tx *VGLutil.Tx, isNew, allowHighFees,
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *VGLutil.Tx, isNew bool) ([]wire.OutPoint, error) {
	// Create agenda flags for checking transactions based on which ones are
	// active or should otherwise always be enforced.
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransactions(txns []*VGLutil.Tx) error {
	// Create agenda flags for checking transactions based on which ones are
	// active or should otherwise always be enforced.
	checkTxFlags, err := mp.determineCheckTxFlags()
	if err != nil {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *VGLutil.Tx, checkTxFlags blockchain.AgendaFlags) []*VGLutil.Tx {
	var acceptedTxns []*VGLutil.Tx
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *VGLutil.Tx, allowOrphan, allowHighFees bool, tag Tag) ([]*VGLutil.Tx, error) {
	// Create agenda flags for checking transactions based on which ones are
	// active or should otherwise always be enforced.
	checkTxFlags, err := mp.determineCheckTxFlags()
	if err != nil {
...
func New(cfg *Config) *TxPool {
	mp := &TxPool{
		...
		transient:       make(map[chainhash.Hash]*VGLutil.Tx),
	}

	// for a given transaction, scan the mempool to find which transactions
	// spend it.
	forEachRedeemer := func(tx *VGLutil.Tx, f func(redeemerTx *mining.TxDesc)) {
