...
// throughout the tests.  They match the Vigil regression test network params
...
func ticketsInBlock(bl *VGLutil.Block) []chainhash.Hash {
...
func ticketsSpentInBlock(bl *VGLutil.Block) []chainhash.Hash {
...
func revokedTicketsInBlock(bl *VGLutil.Block) []chainhash.Hash {
...
func TestTicketDBLongChain(t *testing.T) {
	...
	testBlockchain := make(map[int64]*VGLutil.Block, len(testBlockchainBytes))
	for k, v := range testBlockchainBytes {
		bl, err := VGLutil.NewBlockFromBytes(v)
		if err != nil {
