...
const (
	...
	// These constants are defined here to avoid a dependency on VGLutil.  They
	// are used in consensus code which can't be changed without a vote anyway,
	// so not referring to them directly via VGLutil is safe.
	//
...
func CheckTransactionSanity(tx *wire.MsgTx, maxTxSize uint64) error {
	...
	// Vigil is a quantity of atoms as defined by the AtomsPerCoin constant.
	...
	for _, txOut := range tx.TxOut {
		...
		// detected and reported.  This is impossible for Vigil, but perhaps
