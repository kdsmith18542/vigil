...
type SubsidyParams interface {
	...
	// modified value defined in VGLP0010.
	//
	...
	// effect prior to the modified value defined in VGLP0010.  See the
...
// modifed value defined in VGLP0010.
//
...
//
// Deprecated: Use CalcWorkSubsidyV3 instead.
func (c *SubsidyCache) CalcWorkSubsidyV2(height int64, voters uint16, useVGLP0010 bool) int64 {
	if !useVGLP0010 {
...
const (
	...
	// SSVVGLP0010 specifies the modified subsidy split specified by VGLP0010.
...
func (c *SubsidyCache) CalcWorkSubsidyV3(height int64, voters uint16, splitVariant SubsidySplitVariant) int64 {
	switch splitVariant {
	case SSVVGLP0010:
		// The work subsidy proportion defined in VGLP0010 is 10%.  Thus it is 10
		// since 10/100 = 10%.
		//
	...
	case SSVVGLP0012:
		// The work subsidy proportion defined in VGLP0012 is 1%.  Thus it is 1
		// since 1/100 = 1%.
		//
		// Note that the value is hard coded here as opposed to using the
		// subsidy params in order to avoid the need for a major module bump
...
// block using either the original subsidy split that was in effect at Vigil
// launch or the modified subsidy split defined in VGLP0010 according to the
// provided flag.
//
// It is calculated as a proportion of the total subsidy and max potential
// number of votes per block.
...
//
// Deprecated: Use CalcStakeVoteSubsidyV3 instead.
func (c *SubsidyCache) CalcStakeVoteSubsidyV2(height int64, useVGLP0010 bool) int64 {
	if !useVGLP0010 {
