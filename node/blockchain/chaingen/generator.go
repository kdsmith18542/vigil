...
func (s *SpendableOut) Amount() VGLutil.Amount {
	return s.amount
}
...
func makeSpendableOutForTxInternal(tx *wire.MsgTx, blockHeight, txIndex, txOutIndex uint32, tree int8) SpendableOut {
	return SpendableOut{
		...
		blockIndex:  txIndex,
		amount:      VGLutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func (g *Generator) calcFullSubsidy(blockHeight uint32) VGLutil.Amount {
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoWSubsidy(fullSubsidy VGLutil.Amount, blockHeight uint32, numVotes uint16) VGLutil.Amount {
	const (
		powProportion    = 6
		totalProportions = 10
	)
	...
	ticketsPerBlock := VGLutil.Amount(g.params.TicketsPerBlock)
	return (powSubsidy * VGLutil.Amount(numVotes)) / ticketsPerBlock
}
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcPoSSubsidy(heightVotedOn uint32) VGLutil.Amount {
...
// good tests which exercise that code, so it wouldn't make sense to use the
// same code to generate them.
func (g *Generator) calcDevSubsidy(fullSubsidy VGLutil.Amount, blockHeight uint32, numVotes uint16) VGLutil.Amount {
	const (
		devProportion    = 1
		totalProportions = 10
	)
	...
	// Reduce the subsidy according to the number of votes.
	ticketsPerBlock := VGLutil.Amount(g.params.TicketsPerBlock)
	return (devSubsidy * VGLutil.Amount(numVotes)) / ticketsPerBlock
}
...
func (g *Generator) addCoinbaseTxOutputs(tx *wire.MsgTx, blockHeight uint32, devSubsidy, powSubsidy VGLutil.Amount) {
...
func (g *Generator) CreateTicketPurchaseTx(spend *SpendableOut, ticketPrice, fee VGLutil.Amount) *wire.MsgTx {
...
func (g *Generator) CreateTreasuryTAdd(spend *SpendableOut, amount, fee VGLutil.Amount) *wire.MsgTx {
...
func (g *Generator) CreateTreasuryTSpend(privKey []byte, payouts []AddressAmountTuple, fee VGLutil.Amount, expiry uint32) *wire.MsgTx {
...
func (g *Generator) CreateVoteTx(voteBlock *wire.MsgBlock, ticketTx *wire.MsgTx, ticketBlockHeight, ticketBlockIndex uint32) *wire.MsgTx {
	...
	voteSubsidy := posSubsidy / VGLutil.Amount(g.params.TicketsPerBlock)
	ticketPrice := VGLutil.Amount(ticketTx.TxOut[0].Value)
...
//
// This compact form is only used in Vigil to encode unsigned 256-bit numbers
...
func calcASERTDiff(startDiffBits uint32, powLimit *big.Int, targetSecsPerBlock,
	...
	//
	// Per VGLP0011, the goal equation is:
	//
...
// the ASERT difficulty algorithm defined in VGLP0011.
...
func (g *Generator) CalcNextReqStakeDifficulty(prevBlock *wire.MsgBlock) int64 {
	...
	//
	// Per VGLP0001, the generalized equation is:
	//
...
// will be nil.
func buildMerkleTreeStore(transactions []*VGLutil.Tx) []*chainhash.Hash {
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	utilTxns := make([]*VGLutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, VGLutil.NewTx(tx))
	}
...
// NOTE: This must only be used as a munger to the 'NextBlock' function or it
// will lead to an invalid live ticket pool.
func ReplaceVoteSubsidies(newSubsidy VGLutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
		for _, stx := range b.STransactions {
			if isVoteTx(stx) {
...
func (g *Generator) CreateSpendTx(spend *SpendableOut, fee VGLutil.Amount) *wire.MsgTx {
...
// opTrueRedeemScript.
func (g *Generator) CreateSpendTxForTx(tx *wire.MsgTx, blockHeight, txIndex uint32, fee VGLutil.Amount) *wire.MsgTx {
...
func (g *Generator) NextBlock(blockName string, spend *SpendableOut, ticketSpends []SpendableOut, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	ticketPrice := VGLutil.Amount(g.CalcNextRequiredStakeDifficulty())
	...
	if nextHeight > uint32(g.params.CoinbaseMaturity) {
		...
		// outputs.
		if ticketSpends != nil {
			const ticketFee = VGLutil.Amount(2)
	...
	var stakeTreeFees VGLutil.Amount
	for _, tx := range stakeTxns {
		...
		var inputSum, outputSum VGLutil.Amount
		for _, txIn := range tx.TxIn {
			inputSum += VGLutil.Amount(txIn.ValueIn)
		}
		for _, txOut := range tx.TxOut {
			outputSum += VGLutil.Amount(txOut.Value)
		}
...
func (g *Generator) CreateBlockOne(blockName string, additionalAmount VGLutil.Amount, mungers ...func(*wire.MsgBlock)) *wire.MsgBlock {
	...
	var totalSubsidy VGLutil.Amount
	for _, payout := range g.params.BlockOneLedger {
		...
		totalSubsidy += VGLutil.Amount(payout.Amount)
	}
