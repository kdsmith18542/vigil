...
// subsidy.
func additionalCoinbasePoW(amount VGLutil.Amount) func(*wire.MsgBlock) {
...
func additionalCoinbaseDev(amount VGLutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
		// Increase the first proof-of-work coinbase subsidy by the
		// provided amount.
...
func additionalSpendFee(fee VGLutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	g.NextBlock("bv16", outs[9], ticketOuts[9], func(b *wire.MsgBlock) {
		...
		ticketPrice := VGLutil.Amount(g.CalcNextReqStakeDifficulty(g.Tip()))
	...
	g.NextBlock("bshso1", outs[13], ticketOuts[13], func(b *wire.MsgBlock) {
		...
		for i := 0; i < txnsNeeded; i++ {
			...
			sig, err := sign.RawTxInSignature(tx, 0, redeemScript,
				txscript.SigHashAll, privKeyBytes, VGLec.STEcdsaSecp256k1)
			if err != nil {
				panic(err)
			}
	...
	// vector is not possible in Vigil since the block size is included in
	...
	// Vigil enforces the coinbase includes the block height to which it
	...
	g.NextBlock("bcb9", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = VGLutil.MaxAmount + 1
	})
	rejected(ErrBadTxOutValue)
	...
	g.NextBlock("bcb10", outs[19], ticketOuts[19], func(b *wire.MsgBlock) {
		b.Transactions[1].TxOut[0].Value = VGLutil.MaxAmount
