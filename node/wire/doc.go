...
/*
Package wire implements the Vigil wire protocol.

For the complete details of the Vigil protocol, see the official wiki entry
...
supported Vigil messages to and from the wire. This package does not deal
with the specifics of message handling such as what to do when a message is
received. This provides the caller with a high level of flexibility.

# Vigil Message Overview

The Vigil protocol consists of exchanging messages between peers. Each
message is preceded by a header which identifies information about it such as
which Vigil network it is a part of, its type, how big it is, and a checksum
...
of the currently supported Vigil messages are provided. For these supported
messages, all of the details of marshalling and unmarshalling to and from the
wire using Vigil encoding are handled so the caller doesn't have to concern
themselves with the specifics.

# Message Interaction

The following provides a quick summary of how the Vigil messages are intended
to interact with one another. As stated above, these interactions are not
directly handled by this package. For more in-depth details about the
appropriate interactions, see the official Vigil protocol wiki entry at
...
and write Vigil messages. The following sections provide a quick overview of
...
The Vigil network is a magic number which is used to identify the start of a
message and which Vigil network the message applies to. This package provides
...
As discussed in the Vigil message overview section, this package reads
and writes Vigil messages using a generic interface named Message. In
...
In order to unmarshall Vigil messages from the wire, use the ReadMessage
function.  It accepts any io.Reader, but typically this will be a net.Conn to
a remote node running a Vigil peer.  Example syntax is:

	// Reads and validates the next Vigil message from conn using the
	// protocol version pver and the Vigil network btcnet.  The returns
...
In order to marshall Vigil messages to the wire, use the WriteMessage
function.  It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a Vigil peer.  Example syntax to request addresses
from a remote peer is:
	...
	// pver, and the Vigil network btcnet.  The return is a possible
	// error.
