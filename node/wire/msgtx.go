...
// OutPoint defines a Vigil data type that is used to track previous
// transaction outputs.
...
// NewOutPoint returns a new Vigil transaction outpoint point with the
...
// TxIn defines a Vigil transaction input.
type TxIn struct {
	// Non-witness
...
// NewTxIn returns a new Vigil transaction input with the provided
...
// TxOut defines a Vigil transaction output.
type TxOut struct {
	Value    int64
	Version  uint16
	PkScript []byte
...
// NewTxOut returns a new Vigil transaction output with the provided
...
// MsgTx implements the Message interface and represents a Vigil tx message.
...
func writeTxScriptsToMsgTx(msg *MsgTx, totalScriptSize uint64, serType TxSerializeType) {
	...
	//
	// Closures around writing the TxIn and TxOut scripts are used in Vigil
...
// in that BtcDecode decodes from the Vigil wire protocol as it was sent
...
// encodes the transaction to the Vigil wire protocol in order to be sent
// across the network.  The wire encoding can technically differ depending on
// the protocol version and doesn't even really need to match the format of a
// stored transaction at all.  As of the time this comment was written, the
// encoded transaction is the same in both instances, but there is a distinct
...
// NewMsgTx returns a new Vigil tx message that conforms to the Message
...
// WriteOutPoint encodes op to the Vigil protocol encoding for an OutPoint
// to w.
...
// writeTxInPrefix encodes ti to the Vigil protocol encoding for a transaction
...
// writeTxInWitness encodes ti to the Vigil protocol encoding for a transaction
...
// writeTxOut encodes to into the Vigil protocol encoding for a transaction
// output (TxOut) to w.
