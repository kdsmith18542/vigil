...
// Vigil network (magic) 4 bytes + command 12 bytes + payload length 4 bytes +
// checksum 4 bytes.
const MessageHeaderSize = 24

// CommandSize is the fixed size of all commands in the common Vigil message
...
// Message is an interface that describes a Vigil message.  A type that
...
// messageHeader defines the header structure for all Vigil protocol messages.
...
// readMessageHeader reads a Vigil message header from r.
...
// WriteMessageN writes a Vigil Message to w including the necessary header
// information and returns the number of bytes written.    This function is the
// same as WriteMessage except it also returns the number of bytes written.
func WriteMessageN(w io.Writer, msg Message, pver uint32, VGLnet CurrencyNet) (int, error) {
	...
	writeElements(hw, VGLnet, command, uint32(lenp), checksum)
...
// WriteMessage writes a Vigil Message to w including the necessary header
...
func WriteMessage(w io.Writer, msg Message, pver uint32, VGLnet CurrencyNet) error {
	_, err := WriteMessageN(w, msg, pver, VGLnet)
	return err
}

// ReadMessageN reads, validates, and parses the next Vigil Message from r for
// the provided protocol version and Vigil network.  It returns the number of
// bytes read in addition to the parsed Message and raw bytes which comprise the
// message.  This function is the same as ReadMessage except it also returns the
// number of bytes read.
func ReadMessageN(r io.Reader, pver uint32, VGLnet CurrencyNet) (int, Message, []byte, error) {
	...
	// Check for messages from the wrong Vigil network.
	if hdr.magic != VGLnet {
...
// ReadMessage reads, validates, and parses the next Vigil Message from r for
// the provided protocol version and Vigil network.  It returns the parsed
...
func ReadMessage(r io.Reader, pver uint32, VGLnet CurrencyNet) (Message, []byte, error) {
