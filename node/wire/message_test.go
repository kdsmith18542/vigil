...
// Copyright (c) 2015-2024 The Vigil developers
...
func makeHeader(VGLnet CurrencyNet, command string,
	payloadLen uint32, checksum uint32) []byte {

	// The length of a Vigil message header is 24 bytes.
	// 4 byte magic number of the Vigil network + 12 byte command + 4 byte
	// payload length + 4 byte checksum.
	buf := make([]byte, 24)
	binary.LittleEndian.PutUint32(buf, uint32(VGLnet))
...
func TestMessage(t *testing.T) {
	...
	tests := []struct {
		...
		VGLnet CurrencyNet // Network to use for wire encoding
	...
	for i, test := range tests {
		...
		nw, err := WriteMessageN(&buf, test.in, test.pver, test.VGLnet)
		if err != nil {
		...
		nr, msg, _, err := ReadMessageN(rbuf, test.pver, test.VGLnet)
		if err != nil {
	...
	for i, test := range tests {
		// Encode to wire format.
		var buf bytes.Buffer
		err := WriteMessage(&buf, test.in, test.pver, test.VGLnet)
		if err != nil {
			t.Errorf("WriteMessage #%d error %v", i, err)
			continue
		}
		...
		// Decode from wire format.
		rbuf := bytes.NewReader(buf.Bytes())
		msg, _, err := ReadMessage(rbuf, test.pver, test.VGLnet)
		if err != nil {
			t.Errorf("ReadMessage #%d error %v, msg %v", i, err,
				spew.Sdump(msg))
			continue
...
func TestReadMessageWireErrors(t *testing.T) {
	...
	// otherwise accurate.
	badMessageBytes := makeHeader(VGLnet, "addr", 1, 0xeaadc31c)
	...
	discardBytes := makeHeader(VGLnet, "bogus", 15*1024, 0)

	tests := []struct {
		buf     []byte      // Wire encoding
		pver    uint32      // Protocol version for wire encoding
		VGLnet  CurrencyNet // Vigil network for wire encoding
	...
	for i, test := range tests {
		...
		nr, _, _, err := ReadMessageN(r, test.pver, test.VGLnet)
...
func TestWriteMessageWireErrors(t *testing.T) {
	...
	tests := []struct {
		...
		// Command too long.
		{badCommandMsg, pver, VGLnet, 0, wireErr, 0},
		// Force error in payload encode.
		{encodeErrMsg, pver, VGLnet, 0, wireErr, 0},
		// Force error due to exceeding max overall message payload size.
		{exceedOverallPayloadErrMsg, pver, VGLnet, 0, wireErr, 0},
		// Force error due to exceeding max payload for message type.
		{exceedPayloadErrMsg, pver, VGLnet, 0, wireErr, 0},
		// Force error in header write.
		{bogusMsg, pver, VGLnet, 0, io.ErrShortWrite, 0},
		// Force error in payload write.
		{bogusMsg, pver, VGLnet, 24, io.ErrShortWrite, 24},
	}
	...
	for i, test := range tests {
		...
		nw, err := WriteMessageN(w, test.msg, test.pver, test.VGLnet)
