...
//   os.Setenv("vgld_ALT_DNSNAMES", "hostname1,hostname2")
//
...
func TestLoadConfig(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("Failed to load vgld config: %s", err)
	}
}
...
// TestAltDNSNamesWithEnv ensures the vgld_ALT_DNSNAMES environment variable is
// parsed into a slice of additional hostnames as intended.
func TestAltDNSNamesWithEnv(t *testing.T) {
	appName := filepath.Base(os.Args[0])
	appName = strings.TrimSuffix(appName, filepath.Ext(appName))
	t.Setenv("vgld_ALT_DNSNAMES", "hostname1,hostname2")
	cfg, _, err := loadConfig(appName)
	if err != nil {
		t.Fatalf("Failed to load vgld config: %s", err)
	}
