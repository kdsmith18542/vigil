...
// Tx returns a wrapped transaction (VGLutil.Tx) for the transaction at the
...
// STx returns a wrapped transaction (VGLutil.Tx) for the stake transaction at
...
// Transactions returns a slice of wrapped transactions (VGLutil.Tx) for all
...
// instead provides easy access to wrapped versions (VGLutil.Tx) of them.
...
// STransactions returns a slice of wrapped stake transactions (VGLutil.Tx) for all
// stake transactions in the Block.  This is nearly equivalent to accessing the raw
// transactions (VGLwire.MsgTx) in the underlying wire.MsgBlock, however it
