...
func mkGetKey(keys map[string]addressToKey) KeyDB {
	if keys == nil {
		return KeyClosure(func(addr stdaddr.Address) ([]byte,
			VGLec.SignatureType, bool, error) {
...
func TestSignTxOutput(t *testing.T) {
	...
	signatureSuites := []VGLec.SignatureType{
		VGLec.STEcdsaSecp256k1,
		VGLec.STEd25519,
		VGLec.STSchnorrSecp256k1,
	}
	tx := &wire.MsgTx{
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			if err := signBadAndCheck(msg, tx, i, pkScript, hashType,
				mkGetKey(map[string]addressToKey{
					address.String(): {keyDB, VGLec.STEcdsaSecp256k1, false},
				}), mkGetScript(nil), noTreasury); err == nil {
				t.Errorf("corrupted signature validated: %s", msg)
				break
			}
	...
	for _, hashType := range hashTypes {
		for i := range tx.TxIn {
			...
			// Without treasury agenda.
			suite1 := VGLec.STEcdsaSecp256k1
			suite2 := VGLec.STEcdsaSecp256k1
			if err := signAndCheck(msg, tx, i, scriptPkScript,
				hashType,
				mkGetKey(map[string]addressToKey{
	...
	for i := range sigScriptTests {
		...
		for j := range tx.TxIn {
			...
			script, err = SignatureScript(tx, idx,
				...
				sigScriptTests[i].hashType, privKeyD, VGLec.STEcdsaSecp256k1,
