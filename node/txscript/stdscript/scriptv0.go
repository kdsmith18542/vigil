...
func ExtractPubKeyAltDetailsV0(script []byte) ([]byte, VGLec.SignatureType) {
	...
	if len(script) == 35 && script[0] == txscript.OP_DATA_32 &&
		...
		txscript.AsSmallInt(script[33]) == VGLec.STEd25519 {

		return script[1:33], VGLec.STEd25519
	}

	if len(script) == 36 && script[0] == txscript.OP_DATA_33 &&
		txscript.IsSmallInt(script[34]) &&
		txscript.AsSmallInt(script[34]) == VGLec.STSchnorrSecp256k1 &&
		txscript.IsStrictCompressedPubKeyEncoding(script[1:34]) {

		return script[1:34], VGLec.STSchnorrSecp256k1
	}
...
func ExtractPubKeyEd25519V0(script []byte) []byte {
	...
	if pk == nil || sigType != VGLec.STEd25519 {
		return nil
	}
	return pk
}
...
func ExtractPubKeySchnorrSecp256k1V0(script []byte) []byte {
	pk, sigType := ExtractPubKeyAltDetailsV0(script)
	if pk == nil || sigType != VGLec.STSchnorrSecp256k1 {
		return nil
	}
	return pk
}
...
func IsStandardAltSignatureTypeV0(op byte) bool {
	...
	return sigType == VGLec.STEd25519 || sigType == VGLec.STSchnorrSecp256k1
}
...
// script.  It will return nil otherwise.
func ExtractPubKeyHashAltDetailsV0(script []byte) ([]byte, VGLec.SignatureType) {
	...
	if len(script) == 26 &&
		...
		return script[3:23], VGLec.SignatureType(txscript.AsSmallInt(script[24]))
	}
...
func ExtractPubKeyHashEd25519V0(script []byte) []byte {
	...
	if pkHash == nil || sigType != VGLec.STEd25519 {
		return nil
	}
	return pkHash
}
...
func ExtractPubKeyHashSchnorrSecp256k1V0(script []byte) []byte {
	pkHash, sigType := ExtractPubKeyHashAltDetailsV0(script)
	if pkHash == nil || sigType != VGLec.STSchnorrSecp256k1 {
		return nil
	}
	return pkHash
}
...
//
// NOTE: Atomic swaps are not considered standard script types by the vgld
