...
/*
Package rpcclient implements a websocket-enabled Vigil JSON-RPC client.
...
All notifications provided by vgld require registration to opt-in.  For example,
...
detected by type asserting the error in a *VGLjson.RPCError.  For example, to
detect if a command is unimplemented by the remote RPC server:

	block, err := client.GetBlock(ctx, blockHash)
	if err != nil {
		if jerr, ok := err.(*VGLjson.RPCError); ok {
			switch jerr.Code {
			case VGLjson.ErrRPCUnimplemented:
