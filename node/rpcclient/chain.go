...
func (c *Client) GetBlockAsync(ctx context.Context, blockHash *chainhash.Hash) *FutureGetBlockResult {
	...
	cmd := chainjson.NewGetBlockCmd(hash, VGLjson.Bool(false), nil)
...
func (c *Client) GetBlockVerboseAsync(ctx context.Context, blockHash *chainhash.Hash, verboseTx bool) *FutureGetBlockVerboseResult {
	...
	cmd := chainjson.NewGetBlockCmd(hash, VGLjson.Bool(true), &verboseTx)
...
func (c *Client) GetBlockHeaderAsync(ctx context.Context, hash *chainhash.Hash) *FutureGetBlockHeaderResult {
	cmd := chainjson.NewGetBlockHeaderCmd(hash.String(), VGLjson.Bool(false))
...
func (c *Client) GetBlockHeaderVerboseAsync(ctx context.Context, hash *chainhash.Hash) *FutureGetBlockHeaderVerboseResult {
	cmd := chainjson.NewGetBlockHeaderCmd(hash.String(), VGLjson.Bool(true))
...
// Receive waits for the response promised by the future and returns the
// current coin supply.
func (r *FutureGetCoinSupplyResult) Receive() (VGLutil.Amount, error) {
	res, err := receiveFuture(r.ctx, r.c)
	if err != nil {
		return 0, err
	}
...
func (c *Client) GetCoinSupply(ctx context.Context) (VGLutil.Amount, error) {
...
func (c *Client) GetRawMempoolAsync(ctx context.Context, txType chainjson.GetRawMempoolTxTypeCmd) *FutureGetRawMempoolResult {
	cmd := chainjson.NewGetRawMempoolCmd(VGLjson.Bool(false),
		VGLjson.String(string(txType)))
...
func (c *Client) GetRawMempoolVerboseAsync(ctx context.Context, txType chainjson.GetRawMempoolTxTypeCmd) *FutureGetRawMempoolVerboseResult {
	cmd := chainjson.NewGetRawMempoolCmd(VGLjson.Bool(true),
		VGLjson.String(string(txType)))
...
// ValidateAddress returns information about the given Vigil address.
...
// EstimateSmartFee returns an estimation of a transaction fee rate (in VGL/KB)
...
//
// As of 2019-01, only the default conservative mode is supported by vgld.
