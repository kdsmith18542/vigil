...
// Client represents a Vigil RPC client which allows easy access to the
// various RPC methods available on a Vigil RPC server.  Each of the wrapper
...
type (
	...
	rawResponse struct {
		...
		Error  *VGLjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled VGLjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) sendCmd(ctx context.Context, cmd interface{}) *cmdRes {
	...
	method, err := VGLjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := VGLjson.MarshalCmd("1.0", id, cmd)
	if err != nil {
		return newFutureError(ctx, err)
	}
