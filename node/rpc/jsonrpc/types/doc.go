...
VGLjson.MarshalCmd and VGLjson.MarshalResponse functions may be used.  They
return the raw bytes ready to be sent across the wire.

Unmarshalling a received Request object is a two step process:
 1. Unmarshal the raw bytes into a VGLjson.Request struct instance via
    json.Unmarshal
 2. Use VGLjson.ParseParams on the Method and Params fields of the unmarshalled
...
Unmarshalling a received Response object is also a two step process:
 1. Unmarshal the raw bytes into a VGLjson.Response struct instance via
    json.Unmarshal
...
The second approach is the VGLjson.NewCmd function which takes a method
(command) name and variable arguments.  Since this package registers all of its
types with VGLjson, the function will recognize them and includes full checking
...
To facilitate providing consistent help to users of the RPC server, the VGLjson
...
In addition, the VGLjson.MethodUsageText function may be used to generate
