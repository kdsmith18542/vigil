...
func (s *profileServer) Start(listenAddr string, allowNonLoopback bool) error {
	...
	for listenerIdx := range listeners {
		...
		vgldLog.Infof("Profiling server listening on %s", listener.Addr())
		s.wg.Add(1)
		go func(httpServer *http.Server) {
			...
			if !errors.Is(err, http.ErrServerClosed) {
				vgldLog.Errorf("Profiling server listening on %s exited with "+
...
func (s *profileServer) Stop() error {
	...
	s.wg.Wait()
	if err != nil {
		vgldLog.Errorf("Profiling server stopped with unexpected error: %v",
			err)
		return err
	}

	vgldLog.Info("Profiling server stopped")
	return nil
}
