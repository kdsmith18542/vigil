...
import (
	...
	_ "github.com/vigilnetwork/vgl/database/v3/ffldb"
	...
	"github.com/vigilnetwork/vgl/sampleconfig"
...
const (
	...
	defaultConfigFilename   = "vgld.conf"
	defaultDataDirname      = "data"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "vgld.log"
	defaultLogSize          = "10M"
	defaultDbType           = "ffldb"
...
var (
	...
	defaultHomeDir    = VGLutil.AppDataDir("vgld", false)
...
// config defines the configuration options for vgld.
//
// See loadConfig for details on the configuration load process.
type config struct {
	// General application behavior.
	ShowVersion      bool   `short:"V" long:"version" description:"Display version information and exit"`
	HomeDir          string `short:"A" long:"appdata" description:"Path to application home directory" env:"vgld_APPDATA"`
	...
	AltDNSNames          []string `long:"altdnsnames" description:"Specify additional DNS names to use when generating the RPC server certificate" env:"vgld_ALT_DNSNAMES" env-delim:","`
	...
	ExternalIPs    []string `long:"externalip" description:"Add a public-facing IP to the list of local external IPs that vgld will advertise to other peers"`
	...
	MinRelayTxFee    float64 `long:"minrelaytxfee" description:"The minimum transaction fee in VGL/kB to be considered a non-zero fee"`
...
// createDefaultConfig copies the file sample-vgld.conf to the given destination path,
...
//
// The above results in vgld functioning properly without any config settings
...
func loadConfig(appName string) (*config, []string, error) {
	...
	// Update the home directory for vgld if specified. Since the home
	...
	cfg.minRelayTxFee, err = VGLutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	for _, oldDir := range oldTestNets {
		if fileExists(oldDir) {
			vgldLog.Warnf("Block chain data from previous testnet"+
	...
	if configFileError != nil {
		vgldLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// vgldDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func vgldDial(ctx context.Context, network, addr string) (net.Conn, error) {
...
// vgldLookup invokes the correct DNS lookup function to use depending on the
...
func vgldLookup(host string) ([]net.IP, error) {
