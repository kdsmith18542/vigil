...
import (
	...
	"github.com/vigilnetwork/vgl/wallet/rpc/client/vgld"
	"github.com/vigilnetwork/vgl/wallet/rpc/jsonrpc/types"
	"github.com/vigilnetwork/vgl/wallet/spv"
	"github.com/vigilnetwork/vgl/wallet/version"
	"github.com/vigilnetwork/vgl/wallet/wallet"
	"github.com/vigilnetwork/vgl/wallet/wallet/txauthor"
...
var handlers = map[string]handler{
	...
	"validatepreVGLP0005cf":      {fn: (*Server).validatePreVGLP0005CF},
...
func unimplemented(*Server, context.Context, any) (any, error) {
	return nil, &VGLjson.RPCError{
		Code:    VGLjson.ErrRPCUnimplemented,
...
// context.
type lazyHandler func() (any, *VGLjson.RPCError)
...
func lazyApplyHandler(s *Server, ctx context.Context, request *VGLjson.Request) lazyHandler {
	handlerData, ok := handlers[request.Method]
	if !ok {
		return func() (any, *VGLjson.RPCError) {
			...
			if !ok {
				return nil, rpcErrorf(VGLjson.ErrRPCClientNotConnected, "RPC passthrough requires vgld RPC synchronization")
			}
	...
	return func() (any, *VGLjson.RPCError) {
		params, err := VGLjson.ParseParams(types.Method(request.Method), request.Params)
		if err != nil {
			return nil, VGLjson.ErrRPCInvalidRequest
		}
...
func makeResponse(id, result any, err error) VGLjson.Response {
	idPtr := idPointer(id)
	if err != nil {
		return VGLjson.Response{
			ID:    idPtr,
...
func (s *Server) abandonTransaction(ctx context.Context, icmd any) (any, error) {
	...
	if err != nil {
		return nil, rpcError(VGLjson.ErrRPCDecodeHexString, err)
	}
...
func (s *Server) accountAddressIndex(ctx context.Context, icmd any) (any, error) {
	...
	case udb.InternalBranch:
		return intChild, nil
	default:
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "invalid branch %v", cmd.Branch)
	}
}
...
func (s *Server) accountSyncAddressIndex(ctx context.Context, icmd any) (any, error) {
	...
	if index >= hdkeychain.HardenedKeyStart {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter,
...
func walletPubKeys(ctx context.Context, w *wallet.Wallet, keys []string) ([][]byte, error) {
	...
	for i, key := range keys {
		...
		default:
			...
			return nil, rpcError(VGLjson.ErrRPCInvalidAddressOrKey, err)
		}
		pubKeys[i] = pubKey
	}
...
func (s *Server) addTransaction(ctx context.Context, icmd any) (any, error) {
	...
	if err != nil {
		return nil, rpcError(VGLjson.ErrRPCDeserialization, err)
	}
...
func (s *Server) createRawTransaction(ctx context.Context, icmd any) (any, error) {
	...
	if cmd.Expiry != nil && *cmd.Expiry < 0 {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "Expiry out of range")
	}
	...
	if cmd.LockTime != nil &&
		...
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "Locktime out of range")
	}
	...
	for _, input := range cmd.Inputs {
		...
		if err != nil {
			return nil, rpcError(VGLjson.ErrRPCInvalidParameter, err)
		}
		...
		amt, err := VGLutil.NewAmount(input.Amount)
		if err != nil {
			return nil, rpcError(VGLjson.ErrRPCInvalidParameter, err)
		}
		if amt < 0 {
	...
	for encodedAddr, amount := range cmd.Amounts {
		...
		if err != nil {
			return nil, rpcErrorf(VGLjson.ErrRPCInvalidAddressOrKey,
		...
		atomic, err := VGLutil.NewAmount(amount)
		if err != nil {
			return nil, rpcErrorf(VGLjson.ErrRPCInternal.Code,
				"New amount: %v", err)
		}
		// Ensure amount is in the valid range for monetary amounts.
		if atomic <= 0 || atomic > VGLutil.MaxAmount {
			return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter,
...
func (s *Server) createSignature(ctx context.Context, icmd any) (any, error) {
	...
	if cmd.InputIndex >= len(tx.TxIn) {
		return nil, rpcErrorf(VGLjson.ErrRPCMisc,
...
func (s *Server) discoverUsage(ctx context.Context, icmd any) (any, error) {
	...
	if cmd.StartBlock != nil {
		...
		if err != nil {
			return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "startblock: %v", err)
		}
		startBlock = *h
	}
...
func (s *Server) fundRawTransaction(ctx context.Context, icmd any) (any, error) {
	...
	if cmd.Options != nil {
		...
		if opts.FeeRate != nil {
			feeRate, err = VGLutil.NewAmount(*opts.FeeRate)
			if err != nil {
				return nil, err
			}
		}
	...
	for i := range tx.TxOut {
		fee -= VGLutil.Amount(tx.TxOut[i].Value)
	}
...
func (s *Server) getAddressesByAccount(ctx context.Context, icmd any) (any, error) {
	...
	appendAddrs := func(branchKey *hdkeychain.ExtendedKey, n uint32) error {
		for i := uint32(0); i < n; i++ {
			...
			pkh := VGLutil.Hash160(child.SerializedPubKey())
...
func (s *Server) getBalance(ctx context.Context, icmd any) (any, error) {
	...
	if minConf < 0 {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "minconf must be non-negative")
	}
	...
	if accountName == "*" {
		...
		var (
			totImmatureCoinbase VGLutil.Amount
			totImmatureStakegen VGLutil.Amount
			totLocked           VGLutil.Amount
			totSpendable        VGLutil.Amount
			totUnconfirmed      VGLutil.Amount
			totVotingAuthority  VGLutil.Amount
			cumTot              VGLutil.Amount
		)
		...
		for _, bal := range balances {
			...
			if err != nil {
				...
				if errors.Is(err, errors.NotExist) {
					return nil, rpcError(VGLjson.ErrRPCInternal.Code, err)
				}
				return nil, err
			}
...
func (s *Server) getBestBlock(ctx context.Context, icmd any) (any, error) {
	...
	result := &vgldtypes.GetBestBlockResult{
		Hash:   hash.String(),
		Height: int64(height),
	}
	return result, nil
...
func (s *Server) getBlockHeader(ctx context.Context, icmd any) (any, error) {
	...
	// Attempt RPC passthrough if connected to vgld.
	...
	if cmd.Verbose == nil || !*cmd.Verbose {
		...
		if err != nil {
			return nil, rpcErrorf(VGLjson.ErrRPCInternal.Code, "Could not serialize block header: %v", err)
		}
	...
	if err != nil {
		return nil, rpcErrorf(VGLjson.ErrRPCInternal.Code, "Error checking if block is in mainchain: %v", err)
	}
	if mainChainHasBlock {
		...
		if blockHeight < bestHeight {
			...
			if err != nil {
				return nil, rpcErrorf(VGLjson.ErrRPCInternal.Code, "Info not found for next block: %v", err)
			}
	...
	// with the requested block, which is consistent with vgld.
	...
	for i := 0; i < cap(timestamps); i++ {
		...
		if err != nil {
			return nil, rpcErrorf(VGLjson.ErrRPCInternal.Code, "Info not found for previous block: %v", err)
		}
	}
	...
	// block hash, this is assumed to be v2 (VGLP0011).  More advanced
	...
	return &vgldtypes.GetBlockHeaderVerboseResult{
		...
		SBits:         VGLutil.Amount(blockHeader.SBits).ToCoin(),
...
func (s *Server) getBlock(ctx context.Context, icmd any) (any, error) {
	...
	if len(blocks) == 0 {
		...
		// the following code.
		return nil, rpcErrorf(VGLjson.ErrRPCInternal.Code, "Network returned 0 blocks")
	}
	...
	if cmd.Verbose == nil || !*cmd.Verbose {
		...
		if err != nil {
			return nil, rpcErrorf(VGLjson.ErrRPCInternal.Code, "Could not serialize block: %v", err)
		}
		return b.String(), nil
	}
	...
	sbitsFloat := float64(blockHeader.SBits) / VGLutil.AtomsPerCoin
	blockReply := vgldtypes.GetBlockVerboseResult{
		Hash:          cmd.Hash,
		PoWHash:       powHash.String(),
		Version:       blockHeader.Version,
		MerkleRoot:    blockHeader.MerkleRoot.String(),
	...
	if cmd.VerboseTx == nil || !*cmd.VerboseTx {
		...
		rawTxns := make([]vgldtypes.TxRawResult, len(txns))
		for i, tx := range txns {
			rawTxn, err := createTxRawResult(w.ChainParams(), tx, uint32(i), blockHeader, confirmations, isTreasuryEnabled)
			if err != nil {
				return nil, rpcErrorf(VGLjson.ErrRPCInternal.Code, "Could not create transaction: %v", err)
			}
			rawTxns[i] = *rawTxn
		}
		...
		rawSTxns := make([]vgldtypes.TxRawResult, len(stxns))
		for i, tx := range stxns {
			rawSTxn, err := createTxRawResult(w.ChainParams(), tx, uint32(i), blockHeader, confirmations, isTreasuryEnabled)
			if err != nil {
				return nil, rpcErrorf(VGLjson.ErrRPCInternal.Code, "Could not create stake transaction: %v", err)
			}
			rawSTxns[i] = *rawSTxn
		}
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx, blkIdx uint32, blkHeader *wire.BlockHeader,
	confirmations int64, isTreasuryEnabled bool) (*vgldtypes.TxRawResult, error) {
...
// transaction.
func createVinList(mtx *wire.MsgTx, isTreasuryEnabled bool) []vgldtypes.Vin {
	...
	vinList := make([]vgldtypes.Vin, len(mtx.TxIn))
	if isTreasuryEnabled && blockchain.IsTreasuryBase(mtx) {
		txIn := mtx.TxIn[0]
		vinEntry := &vinList[0]
		vinEntry.Treasurybase = true
		vinEntry.Sequence = txIn.Sequence
		vinEntry.AmountIn = VGLutil.Amount(txIn.ValueIn).ToCoin()
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.BlockHeight = txIn.BlockHeight
		vinEntry.BlockIndex = txIn.BlockIndex
		vinEntry.ScriptSig = &vgldtypes.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []vgldtypes.Vout {
	txType := stake.DetermineTxType(mtx)
	voutList := make([]vgldtypes.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout vgldtypes.Vout
		voutSPK := &vout.ScriptPubKey
		vout.N = uint32(i)
		vout.Value = VGLutil.Amount(v.Value).ToCoin()
		...
		if commitAmt != nil {
			voutSPK.CommitAmt = VGLjson.Float64(commitAmt.ToCoin())
		}
...
func (s *Server) getInfo(ctx context.Context, icmd any) (any, error) {
	...
	var spendableBalance VGLutil.Amount
	...
	if chainSyncer, ok := n.(*chain.Syncer); ok {
		var consensusInfo vgldtypes.InfoChainResult
...
// appear in the blockchain, or any tx that has arrived in the vgld mempool).
// If the most recently-requested address has been used, a new address (the
// next chained address in the keypool) is used.  This can fail if the keypool
// runs out (and will return VGLjson.ErrRPCWalletKeypoolRanOut if that happens).
...
func (s *Server) importCFiltersV2(ctx context.Context, icmd any) (any, error) {
	...
	for i, fdhex := range cmd.Filters {
		...
		if err != nil {
			return nil, rpcErrorf(VGLjson.ErrRPCInvalidParams.Code, "filter %d is not a valid hex string", i)
		}
	}

	err := w.ImportCFiltersV2(ctx, cmd.StartHeight, filterData)
	if err != nil {
		return nil, rpcError(VGLjson.ErrRPCInvalidRequest.Code, err)
	}
...
func (s *Server) importPrivKey(ctx context.Context, icmd any) (any, error) {
	...
	wif, err := VGLutil.DecodeWIF(cmd.PrivKey, w.ChainParams().PrivateKeyID)
	if err != nil {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidAddressOrKey, "WIF decode failed: %v", err)
	}
...
func (s *Server) importScript(ctx context.Context, icmd any) (any, error) {
	...
	if len(rs) == 0 {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "empty script")
	}
...
func (s *Server) createNewAccount(ctx context.Context, icmd any) (any, error) {
	...
	if err != nil {
		if errors.Is(err, errors.Locked) {
			return nil, rpcErrorf(VGLjson.ErrRPCWalletUnlockNeeded, "creating new accounts requires an unlocked wallet")
		}
		return nil, err
	}
	return nil, nil
...
func (s *Server) getNewAddress(ctx context.Context, icmd any) (any, error) {
	...
	if cmd.GapPolicy != nil {
		...
		default:
			return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "unknown gap policy %q", *cmd.GapPolicy)
		}
	}
...
func (s *Server) getTransaction(ctx context.Context, icmd any) (any, error) {
	...
	if errors.Is(err, errors.NotExist) {
		return nil, rpcErrorf(VGLjson.ErrRPCNoTxInfo, "no information for transaction")
	} else if err != nil {
		return nil, err
	}
	...
	var (
		debitTotal  VGLutil.Amount
		creditTotal VGLutil.Amount
		fee         VGLutil.Amount
		negFeeF64   float64
	)
	...
	if len(txd.Debits) == len(txd.MsgTx.TxIn) {
		var outputTotal VGLutil.Amount
		for _, output := range txd.MsgTx.TxOut {
			outputTotal += VGLutil.Amount(output.Value)
		}
...
func (s *Server) getTxOut(ctx context.Context, icmd any) (any, error) {
	...
	if cmd.Tree != wire.TxTreeRegular && cmd.Tree != wire.TxTreeStake {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "Tx tree must be regular or stake")
	}
	...
	return &vgldtypes.GetTxOutResult{
		BestBlock:     bestHash.String(),
		Confirmations: confirmations,
		Value:         utxo.Amount.ToCoin(),
		ScriptPubKey: vgldtypes.ScriptPubKeyResult{
			Asm:       disbuf,
...
func (s *Server) help(ctx context.Context, icmd any) (any, error) {
	...
	// vgld for additional help methods.  This avoids including websocket-only
	...
	return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "no help for method %q", *cmd.Command)
}
...
func (s *Server) listSinceBlock(ctx context.Context, icmd any) (any, error) {
	...
	if targetConf < 1 {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "target_confirmations must be positive")
	}
...
func (s *Server) purchaseTicket(ctx context.Context, icmd any) (any, error) {
	...
	spendLimit, err := VGLutil.NewAmount(cmd.SpendLimit)
	if err != nil {
		return nil, rpcError(VGLjson.ErrRPCInvalidParameter, err)
	}
	if spendLimit < 0 {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "negative spend limit")
	}
	...
	if cmd.MinConf != nil {
		minConf = int32(*cmd.MinConf)
		if minConf < 0 {
			return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "negative minconf")
		}
	}
...
func (s *Server) processUnmanagedTicket(ctx context.Context, icmd any) (any, error) {
	...
	if cmd.TicketHash == "" {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "ticket hash must be provided")
	}
	...
	if vspHost == "" {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "vsphost must be set in options")
	}
...
// and amounts.
func makeOutputs(pairs map[string]VGLutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
...
// All errors are returned in VGLjson.RPCError format
func (s *Server) sendPairs(ctx context.Context, w *wallet.Wallet, amounts map[string]VGLutil.Amount, account uint32, minconf int32) (string, error) {
	...
	if err != nil {
		...
		if errors.Is(err, errors.InsufficientBalance) {
			return "", rpcError(VGLjson.ErrRPCWalletInsufficientFunds, err)
		}
		return "", err
	}
...
// returned in VGLjson.RPCError format
func (s *Server) sendAmountToTreasury(ctx context.Context, w *wallet.Wallet, amount VGLutil.Amount, account uint32, minconf int32) (string, error) {
	changeAccount := account
	if s.cfg.MixingEnabled {
		mixAccount, err := w.AccountNumber(ctx, s.cfg.MixAccount)
		if err != nil {
	...
	if err != nil {
		...
		if errors.Is(err, errors.InsufficientBalance) {
			return "", rpcError(VGLjson.ErrRPCWalletInsufficientFunds,
				err)
		}
		return "", err
	}
...
func (s *Server) sendOutputsFromTreasury(ctx context.Context, w *wallet.Wallet, cmd types.SendFromTreasuryCmd) (string, error) {
	...
	if err != nil {
		return "", rpcErrorf(VGLjson.ErrRPCInternal.Code,
	...
	var totalPayout VGLutil.Amount
	for address, amount := range cmd.Amounts {
		amt, err := VGLutil.NewAmount(amount)
		if err != nil {
			return "", rpcError(VGLjson.ErrRPCInvalidParameter, err)
		}
		...
		if txrules.IsDustOutput(txOut, w.RelayFee()) {
			return "", rpcErrorf(VGLjson.ErrRPCInvalidParameter,
...
func (s *Server) setDisapprovePercent(ctx context.Context, icmd any) (any, error) {
	if s.activeNet.Net == wire.MainNet {
		return nil, VGLjson.ErrInvalidRequest
	}
	cmd := icmd.(*types.SetDisapprovePercentCmd)
	if cmd.Percent > 100 {
		return nil, rpcError(VGLjson.ErrRPCInvalidParameter,
...
func (s *Server) redeemMultiSigOuts(ctx context.Context, icmd any) (any, error) {
	...
	if !ok {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "address is not P2SH")
	}
...
func spendOutputsInputSource(ctx context.Context, w *wallet.Wallet,
	...
	for i, in := range inputs {
		...
		detail.Amount += VGLutil.Amount(prevOut.Value)
	...
	fn := func(target VGLutil.Amount) (*txauthor.InputDetail, error) {
		return detail, nil
	}
	return fn, nil
}
...
func (s *Server) spendOutputs(ctx context.Context, icmd any) (any, error) {
	...
	for _, output := range cmd.Outputs {
		...
		amount, err := VGLutil.NewAmount(output.Amount)
		if err != nil {
			return nil, rpcError(VGLjson.ErrRPCInvalidParameter, err)
		}
	...
	err := w.GetTickets(ctx, func(ts []*wallet.TicketSummary, h *wire.BlockHeader) (bool, error) {
		for _, t := range ts {
			...
			info := types.TicketInfoResult{
				...
				Cost:        VGLutil.Amount(out.Value).ToCoin(),
				BlockHeight: -1,
...
func (s *Server) sendFrom(ctx context.Context, icmd any) (any, error) {
	...
	if !isNilOrEmpty(cmd.Comment) || !isNilOrEmpty(cmd.CommentTo) {
		return nil, rpcErrorf(VGLjson.ErrRPCUnimplemented, "transaction comments are unsupported")
	}
	...
	if cmd.Amount < 0 {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "negative amount")
	}
	minConf := int32(*cmd.MinConf)
	if minConf < 0 {
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "negative minconf")
	...
	amt, err := VGLutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(VGLjson.ErrRPCInvalidParameter, err)
	}
	pairs := map[string]VGLutil.Amount{
		cmd.ToAddress: amt,
	}
...
func (s *Server) sendMany(ctx context.Context, icmd any) (any, error) {
	...
	// Recreate address/amount pairs, using VGLutil.Amount.
	pairs := make(map[string]VGLutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := VGLutil.NewAmount(v)
		if err != nil {
			return nil, rpcError(VGLjson.ErrRPCInvalidParameter, err)
		}
		pairs[k] = amt
...
func (s *Server) sendToMultiSig(ctx context.Context, icmd any) (any, error) {
	...
	account := uint32(udb.DefaultAccountNum)
	amount, err := VGLutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(VGLjson.ErrRPCInvalidParameter, err)
	}
...
func (s *Server) setTxFee(ctx context.Context, icmd any) (any, error) {
	...
	relayFee, err := VGLutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(VGLjson.ErrRPCInvalidParameter, err)
	}
...
func (s *Server) signRawTransaction(ctx context.Context, icmd any) (any, error) {
	...
	case "ssrtx": // Special case of SigHashAll
		hashType = txscript.SigHashAll
	default:
		return nil, rpcErrorf(VGLjson.ErrRPCInvalidParameter, "invalid sighash flag")
	}

	// TODO: really we probably should look these up with vgld anyway to
	...
	// querying vgld with getrawtransaction. We queue up a bunch of async
	// requests and will wait for replies after we have checked the rest of
	// the arguments.
	requested := make(map[wire.OutPoint]*vgldtypes.GetTxOutResult)
	...
	var keys map[string]*VGLutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*VGLutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := VGLutil.DecodeWIF(key, w.ChainParams().PrivateKeyID)
			if err != nil {
				return nil, rpcError(VGLjson.ErrRPCDeserialization, err)
			}
...
func sumOutputValues(outputs []*wire.TxOut) (totalOutput VGLutil.Amount) {
	for _, txOut := range outputs {
		totalOutput += VGLutil.Amount(txOut.Value)
	}
	return totalOutput
}
...
func (s *Server) sweepAccount(ctx context.Context, icmd any) (any, error) {
	...
	if cmd.FeePerKb != nil {
		var err error
		feePerKb, err = VGLutil.NewAmount(*cmd.FeePerKb)
		if err != nil {
			return nil, rpcError(VGLjson.ErrRPCInvalidParameter, err)
		}
	}
	...
	if err != nil {
		if errors.Is(err, errors.InsufficientBalance) {
			return nil, rpcError(VGLjson.ErrRPCWalletInsufficientFunds, err)
		}
		return nil, err
	}
...
// validatePreVGLP0005CF handles the validatepreVGLP0005cf command.
func (s *Server) validatePreVGLP0005CF(ctx context.Context, icmd any) (any, error) {
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	err := w.ValidatePreVGLP0005CFilters(ctx)
	return err == nil, err
}
...
func (s *Server) version(ctx context.Context, icmd any) (any, error) {
	resp := make(map[string]vgldtypes.VersionResult)
	n, _ := s.walletLoader.NetworkBackend()
	if chainSyncer, ok := n.(*chain.Syncer); ok {
...
func (s *Server) walletPassphraseChange(ctx context.Context, icmd any) (any, error) {
	...
	if err != nil {
		if errors.Is(err, errors.Passphrase) {
			return nil, rpcErrorf(VGLjson.ErrRPCWalletPassphraseIncorrect, "incorrect passphrase")
		}
		return nil, err
	}
	return nil, nil
...
func decodeHexStr(hexStr string) ([]byte, error) {
	...
	if err != nil {
		return nil, rpcErrorf(VGLjson.ErrRPCDecodeHexString, "hex string decode failed: %v", err)
	}
	return decoded, nil
}
