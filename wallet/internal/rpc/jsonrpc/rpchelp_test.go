...
func TestRPCMethodHelpGeneration(t *testing.T) {
	...
	for i := range rpchelp.HelpDescs {
		...
		for _, m := range rpchelp.Methods {
			...
			helpText, err := VGLjson.GenerateHelp(types.Method(m.Method), rpchelp.HelpDescs[i].Descs, m.ResultTypes...)
			if err != nil {
...
func TestRPCMethodUsageGeneration(t *testing.T) {
	...
	for _, m := range rpchelp.Methods {
		...
		usage, err := VGLjson.MethodUsageText(types.Method(m.Method))
		if err != nil {
