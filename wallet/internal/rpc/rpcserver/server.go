...
//
//	https://github.com/Vigil/vglwallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//	https://github.com/Vigil/vglwallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
import (
	...
	"github.com/vigilnetwork/vgl/wallet/internal/cfgutil"
	"github.com/vigilnetwork/vgl/wallet/internal/loader"
	"github.com/vigilnetwork/vgl/wallet/internal/netparams"
	...
	"github.com/vigilnetwork/vgl/wallet/walletseed"
	"github.com/vigilnetwork/vgl/addrmgr/v2"
...
// VGLutil.DecodeAddress, which does not perform the network check.
...
// as well as establishing a RPC connection to a vgld consensus server.
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := VGLutil.DecodeWIF(req.PrivateKeyWif, s.wallet.ChainParams().PrivateKeyID)
	if err != nil {
...
func (s *walletServer) SweepAccount(ctx context.Context, req *pb.SweepAccountRequest) (*pb.SweepAccountResponse, error) {
	...
	if req.FeePerKb > 0 {
		var err error
		feePerKb, err = VGLutil.NewAmount(req.FeePerKb)
		if err != nil {
...
func (s *walletServer) BlockInfo(ctx context.Context, req *pb.BlockInfoRequest) (*pb.BlockInfoResponse, error) {
	...
	return &pb.BlockInfoResponse{
		...
		ApprovesParent:   header.VoteBits&VGLutil.BlockValid != 0,
	}, nil
}
...
func (s *walletServer) UnspentOutputs(req *pb.UnspentOutputsRequest, svr pb.WalletService_UnspentOutputsServer) error {
	...
	inputDetail, err := s.wallet.SelectInputs(svr.Context(), VGLutil.Amount(req.TargetAmount), policy)
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	inputDetail, err := s.wallet.SelectInputs(ctx, VGLutil.Amount(req.TargetAmount), policy)
	// Do not return errors to caller when there was insufficient spendable
	// outputs available for the target amount.
	if err != nil && !errors.Is(err, errors.InsufficientBalance) {
		return nil, translateError(err)
	...
	if req.IncludeChangeScript && inputDetail.Amount > VGLutil.Amount(req.TargetAmount) {
...
func (s *walletServer) ConstructTransaction(ctx context.Context, req *pb.ConstructTransactionRequest) (
	*pb.ConstructTransactionResponse, error) {
	...
	if req.FeePerKb != 0 {
		feePerKb = VGLutil.Amount(req.FeePerKb)
	}
...
func (s *walletServer) GetTicket(ctx context.Context, req *pb.GetTicketRequest) (*pb.GetTicketsResponse, error) {
	...
	// The vgld client could be nil here if the network backend is not
...
func (s *walletServer) PurchaseTickets(ctx context.Context,
	...
	// purchase request.
	spendLimit := VGLutil.Amount(req.SpendLimit)
	if spendLimit < 0 {
		return nil, status.Errorf(codes.InvalidArgument,
	...
	txFee := VGLutil.Amount(req.TxFee)
...
func marshalBlock(v *wallet.Block) *pb.BlockDetails {
	...
	return &pb.BlockDetails{
		...
		ApprovesParent: v.Header.VoteBits&VGLutil.BlockValid != 0,
		Transactions:   txs,
...
func (t *ticketbuyerServer) RunTicketBuyer(req *pb.RunTicketBuyerRequest, svr pb.TicketBuyerService_RunTicketBuyerServer) error {
	...
	tb := ticketbuyer.New(w, ticketbuyer.Config{
		...
		Account:            req.Account,
		VotingAccount:      req.VotingAccount,
		Maintain:           VGLutil.Amount(req.BalanceToMaintain),
		VSP:                vspClient,
		Mixing:             req.EnableMixing,
		MixedAccount:       mixedAccount,
