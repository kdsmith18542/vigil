...
var (
	...
	bucketStakeInvalidateVGLedits = []byte("ic")
...
func fetchMinedBalance(ns walletdb.ReadBucket) (VGLutil.Amount, error) {
	v := ns.Get(rootMinedBalance)
	if len(v) != 8 {
		return 0, errors.E(errors.IO, errors.Errorf("mined balance len %d", len(v)))
	}
	return VGLutil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt VGLutil.Amount) error {
	v := make([]byte, 8)
...
func extractRawUnmineVGLeditTxHash(k []byte) []byte {
	return k[:32]
}
...
func fetchRawCreditAmount(v []byte) (VGLutil.Amount, error) {
	if len(v) < 9 {
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (VGLutil.Amount, bool, error) {
	if len(v) < 9 {
		return 0, false, errors.E(errors.IO, errors.Errorf("credit len %d", len(v)))
	}
	return VGLutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (VGLutil.Amount, bool, error) {
	if len(v) < 9 {
		return 0, false, errors.E(errors.IO, errors.Errorf("credit len %d", len(v)))
	}
	return VGLutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spenVGLedit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (VGLutil.Amount, error) {
	...
	return VGLutil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (VGLutil.Amount, error) {
	...
	return VGLutil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func existsInvalidateVGLedit(ns walletdb.ReadBucket, txHash *chainhash.Hash, index uint32, block *Block) (k, v []byte) {
	k = keyCredit(txHash, index, block)
	v = ns.NestedReadBucket(bucketStakeInvalidateVGLedits).Get(k)
	return
}
...
func makeReaVGLeditIterator(ns walletdb.ReadBucket, prefix []byte, dbVersion uint32) creditIterator {
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Amount = VGLutil.Amount(byteOrder.Uint64(it.cv))
...
func valueDebit(amount VGLutil.Amount, credKey []byte) []byte {
	v := make([]byte, 80)
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount VGLutil.Amount, block *Block, credKey []byte) error {
...
func extractRawDebitAmount(v []byte) VGLutil.Amount {
	return VGLutil.Amount(byteOrder.Uint64(v[:8]))
}
...
func valueUnmineVGLedit(amount VGLutil.Amount, change bool, opCode uint8,
...
func putRawUnmineVGLedit(ns walletdb.ReadWriteBucket, k, v []byte) error {
	err := ns.NestedReadWriteBucket(bucketUnmineVGLedits).Put(k, v)
	if err != nil {
		return errors.E(errors.IO, err)
	}
	return nil
...
func fetchRawUnmineVGLeditIndex(k []byte) (uint32, error) {
...
func fetchRawUnmineVGLeditAmount(v []byte) (VGLutil.Amount, error) {
...
func fetchRawUnmineVGLeditAmountChange(v []byte) (VGLutil.Amount, bool, error) {
	if len(v) < unconfValueSizeLegacy {
		return 0, false, errors.E(errors.IO, errors.Errorf("unmined credit len %d", len(v)))
	}
	amt := VGLutil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
	return amt, change, nil
}

func fetchRawUnmineVGLeditTagOpCode(v []byte) uint8 {
	return expandOpCode(v[8])
}

func fetchRawUnmineVGLeditTagIsCoinbase(v []byte) bool {
	return v[8]&(1<<5) != 0
}

func fetchRawUnmineVGLeditScriptType(v []byte) scriptType {
...
func fetchRawUnmineVGLeditScriptOffset(v []byte) uint32 {
	if len(v) < unconfValueSize {
		return 0
	}
...
func fetchRawUnmineVGLeditScriptLength(v []byte) uint32 {
	if len(v) < unconfValueSize {
		return 0
	}
...
func fetchRawUnmineVGLeditAccount(v []byte) (uint32, error) {
	if len(v) < unconfValueSize {
		return 0, errors.E(errors.IO, errors.Errorf("unmined credit len %d", len(v)))
	}
...
func existsRawUnmineVGLedit(ns walletdb.ReadBucket, k []byte) []byte {
	return ns.NestedReadBucket(bucketUnmineVGLedits).Get(k)
}

func deleteRawUnmineVGLedit(ns walletdb.ReadWriteBucket, k []byte) error {
	err := ns.NestedReadWriteBucket(bucketUnmineVGLedits).Delete(k)
	if err != nil {
		return errors.E(errors.IO, err)
	}
	return nil
...
// unmineVGLeditIterator allows for cursor iteration over all credits, in order,
// from a single unmined transaction.
//
//	Example usage:
//
//	 it := makeUnmineVGLeditIterator(ns, txHash)
//	 for it.next() {
...
type unmineVGLeditIterator struct {
	c         walletdb.ReadWriteCursor
	dbVersion uint32
	prefix    []byte
	ck        []byte
...
func makeReadUnmineVGLeditIterator(ns walletdb.ReadBucket, txHash *chainhash.Hash, dbVersion uint32) unmineVGLeditIterator {
	c := ns.NestedReadBucket(bucketUnmineVGLedits).ReadCursor()
	return unmineVGLeditIterator{c: readCursor{c}, prefix: txHash[:], dbVersion: dbVersion}
}

func (it *unmineVGLeditIterator) readElem() error {
	index, err := fetchRawUnmineVGLeditIndex(it.ck)
	if err != nil {
		return err
	}
	amount, change, err := fetchRawUnmineVGLeditAmountChange(it.cv)
	if err != nil {
		return err
	}
...
func (it *unmineVGLeditIterator) next() bool {
	if it.c == nil {
		return false
	}
...
// unavailable until https://github.com/boltdb/bolt/issues/620 is fixed.
// func (it *unmineVGLeditIterator) delete() error {
// 	err := it.c.Delete()
// 	if err != nil {
// 		return errors.E(errors.IO, err)
// 	}
...
func (it *unmineVGLeditIterator) close() {
	if it.c == nil {
		return
	}
	it.c.Close()
...
func valueMultisigOut(sh [ripemd160.Size]byte, m uint8, n uint8,
	...
	blockHeight uint32, amount VGLutil.Amount, spentBy chainhash.Hash,
...
func fetchMultisigOut(k, v []byte) (*MultisigOut, error) {
	...
	mso.Amount = VGLutil.Amount(byteOrder.Uint64(v[59:67]))
...
func fetchMultisigOutAmount(v []byte) VGLutil.Amount {
	return VGLutil.Amount(byteOrder.Uint64(v[59:67]))
}
...
func valueTicketCommitment(amount VGLutil.Amount, account uint32) []byte {
	v := make([]byte, 12)
	byteOrder.PutUint64(v, uint64(amount))
...
func fetchRawTicketCommitmentAmount(v []byte) (VGLutil.Amount, error) {
	if len(v) < 8 {
...
func createStore(ns walletdb.ReadWriteBucket, chainParams *chaincfg.Params) error {
	...
	_, err = ns.CreateBucket(bucketUnmineVGLedits)
	if err != nil {
		return errors.E(errors.IO, err)
	}
	...
	_, err = ns.CreateBucket(bucketStakeInvalidateVGLedits)
	if err != nil {
		return errors.E(errors.IO, err)
	}
