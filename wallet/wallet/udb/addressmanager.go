...
import (
	...
	"github.com/vigilnetwork/vgl/wallet/kdf"
...
// now created using the coin types assigned to Vigil in SLIP0044:
//
...
// to the coin type registered to Vigil as per SLIP0044.  On mainnet, this
...
func (m *Manager) ImportPrivateKey(ns walletdb.ReadWriteBucket, wif *VGLutil.WIF) (ManagedPubKeyAddress, error) {
	defer m.mtx.Unlock()
	m.mtx.Lock()
	...
	pubKeyHash := VGLutil.Hash160(serializedPubKey)
...
func (m *Manager) ImportScript(ns walletdb.ReadWriteBucket, script []byte) (ManagedScriptAddress, error) {
	...
	// Prevent duplicates.
	scriptHash := VGLutil.Hash160(script)
...
func (m *Manager) RecordDerivedAddress(dbtx walletdb.ReadWriteTx, account, branch, child uint32, pubkey []byte) error {
	...
	hash160 := VGLutil.Hash160(pubkey)
