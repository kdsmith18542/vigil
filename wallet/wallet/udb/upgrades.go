...
import (
	...
	"github.com/vigilnetwork/vgl/wallet/wallet/internal/snacl"
...
const (
	...
	// assigned to Vigil in SLIP0044.  The upgrade does not add or remove any
	...
	// store compact filters, which are required for Vigil's SPV
	...
	// mainnet launch, vglwallet has always recorded imported P2SH redeem
...
func lastUsedAddressIndexUpgrade(tx walletdb.ReadWriteTx, publicPassphrase []byte, params *chaincfg.Params) error {
	...
	for account := uint32(0); account <= lastAccount; account++ {
		...
		for child := uint32(0); child < hdkeychain.HardenedKeyStart; child++ {
			...
			// VGLutil.NewAddressPubKeyHash.  Also, while it looks like a
...
func hasExpiryUpgrade(tx walletdb.ReadWriteTx, publicPassphrase []byte, params *chaincfg.Params) error {
	...
	unmineVGLeditsBucket := txmgrBucket.NestedReadWriteBucket(bucketUnmineVGLedits)
	unminedCursor := unmineVGLeditsBucket.ReadWriteCursor()
	unmineVGLeditsKV := map[string][]byte{}
	for k, v := unminedCursor.First(); v != nil; k, v = unminedCursor.Next() {
		hash, err := chainhash.NewHash(extractRawUnmineVGLeditTxHash(k))
		if err != nil {
			unminedCursor.Close()
			return err
		}
		...
		if record.MsgTx.Expiry != wire.NoExpiryValue {
			...
			vCpy[8] |= 1 << 4
			unmineVGLeditsKV[string(k)] = vCpy
		}
	}
	unminedCursor.Close()

	for k, v := range unmineVGLeditsKV {
		err = unmineVGLeditsBucket.Put([]byte(k), v)
		if err != nil {
			return err
		}
	}
...
func hasExpiryFixedUpgrade(tx walletdb.ReadWriteTx, publicPassphrase []byte, params *chaincfg.Params) error {
	...
	// Iterate through all unmined credits
	unmineVGLeditsBucket := txmgrBucket.NestedReadWriteBucket(bucketUnmineVGLedits)
	unminedCursor := unmineVGLeditsBucket.ReadCursor()
	unmineVGLeditsKV := map[string][]byte{}
	for k, v := unminedCursor.First(); v != nil; k, v = unminedCursor.Next() {
		hash, err := chainhash.NewHash(extractRawUnmineVGLeditTxHash(k))
		if err != nil {
		...
		if record.MsgTx.Expiry != wire.NoExpiryValue {
			...
			// Reset OP_SSTXCHANGE flag if this is a ticket purchase
			// OP_SSTXCHANGE output.
			idx, err := fetchRawUnmineVGLeditIndex(k)
			if err != nil {
				unminedCursor.Close()
				return err
			}
...
func unencryptedRedeemScriptsUpgrade(tx walletdb.ReadWriteTx, publicPassphrase []byte, params *chaincfg.Params) error {
	...
	for _, v := cursor.First(); v != nil; _, v = cursor.Next() {
		...
		var hash [20]byte
		copy(hash[:], VGLutil.Hash160(script))
		scripts[hash] = script
	}
	cursor.Close()
