...
func TestStoreQueries(t *testing.T) {
	...
	adVGLedit := func(s *Store, rec *TxRecord, block *BlockMeta, index uint32, change bool) {
		err := s.AdVGLedit(rec, block, index, change, defaultAccount)
		if err != nil {
			t.Fatal(err)
		}
	}
	...
	newState.blocks[0][0].Credits = []CreditRecord{
		{
			Index:  0,
			Amount: VGLutil.Amount(recA.MsgTx.TxOut[0].Value),
			Spent:  false,
			Change: true,
		},
	}
	...
	tests = append(tests, queryTest{
		...
		updates: func() { adVGLedit(s, recA, nil, 0, true) },
		state:   newState,
	})

	// Insert another unmined transaction which spends txA:0, splitting the
	// amount into outputs of 40 and 60 VGL.
	...
	newState.blocks[0][1].Credits = []CreditRecord{
		{
			Index:  0,
			Amount: VGLutil.Amount(recB.MsgTx.TxOut[0].Value),
			Spent:  false,
			Change: false,
		},
	}
	...
	tests = append(tests, queryTest{
		...
		updates: func() { adVGLedit(s, recB, nil, 0, false) },
		state:   newState,
	})
...
func TestPreviousPkScripts(t *testing.T) {
	...
	defaultAccount := uint32(0)
	adVGLedit := func(rec *TxRecord, block *BlockMeta, index uint32) {
		err := s.AdVGLedit(rec, block, index, false, defaultAccount)
		if err != nil {
			t.Fatal(err)
		}
	}
	...
	// returned.
	adVGLedit(recA, nil, 0)
	adVGLedit(recA, nil, 1)
	adVGLedit(recB, nil, 0)
	adVGLedit(recB, nil, 1)
	adVGLedit(recC, nil, 0)
	tests = []scriptTest{
		{recA, nil, nil},
		{recA, &b100.Block, nil},
