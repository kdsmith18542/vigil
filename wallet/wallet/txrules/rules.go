...
const DefaultRelayFeePerKb VGLutil.Amount = 0.0001 * 1e8

// IsDustAmount determines whether a transaction output value and script length would
// cause the output to be considered dust.  Transactions with dust outputs are
// not standard and are rejected by mempools with default policies.
func IsDustAmount(amount VGLutil.Amount, scriptSize int, relayFeePerKb VGLutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb VGLutil.Amount) bool {
	...
	return IsDustAmount(VGLutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
func CheckOutput(output *wire.TxOut, relayFeePerKb VGLutil.Amount) error {
	if output.Value < 0 {
		return errors.E(errors.Invalid, "transaction output amount is negative")
	}
	if output.Value > VGLutil.MaxAmount {
...
func FeeForSerializeSize(relayFeePerKb VGLutil.Amount, txSerializeSize int) VGLutil.Amount {
	fee := relayFeePerKb * VGLutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > VGLutil.MaxAmount {
		fee = VGLutil.MaxAmount
	}
...
func PaysHighFees(totalInput VGLutil.Amount, tx *wire.MsgTx) bool {
...
func TxPaysHighFees(tx *wire.MsgTx) (bool, error) {
	...
	for i, in := range tx.TxIn {
		...
		input += VGLutil.Amount(in.ValueIn)
	}
