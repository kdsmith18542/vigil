...
func (w *Wallet) SelectInputs(ctx context.Context, targetAmount VGLutil.Amount, policy OutputSelectionPolicy) (inputDetail *txauthor.InputDetail, err error) {
...
func (w *Wallet) OutputInfo(ctx context.Context, out *wire.OutPoint) (OutputInfo, error) {
	...
	err := walletdb.View(ctx, w.db, func(dbtx walletdb.ReadTx) error {
		...
		info.Amount = VGLutil.Amount(txDetails.TxRecord.MsgTx.TxOut[out.Index].Value)
