...
import (
	...
	"github.com/vigilnetwork/vgl/wallet/deployments"
	...
	"github.com/vigilnetwork/vgl/wallet/validate"
	...
	"github.com/vigilnetwork/vgl/mixing/mixclient"
...
type Wallet struct {
	...
	tspendKeyPolicy    map[string]stake.TreasuryVoteT // keyed by Vigiliteia key
...
// SetTreasuryKeyPolicy sets a tspend vote policy for a specific Vigiliteia
// instance key.
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee VGLutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
// message or vgld JSON-RPC request, for the blocks in sidechain and saved in
...
func (w *Wallet) CreateMultisigTx(ctx context.Context, account uint32, amount VGLutil.Amount,
...
func (w *Wallet) PurchaseTickets(ctx context.Context, n NetworkBackend,
	...
	_, height := w.MainChainTip(ctx)
	// In SPV mode, VGLP0010 and VGLP0012 are assumed to have activated.  In RPC
	...
	vspFee := txrules.StakePoolTicketFee(sdiff, relayFee, height,
		feePercent, w.chainParams, VGLP0010Active, VGLP0012Active)
	a := &authorTx{
...
// spending to it in the blockchain or vgld mempool), the next chained address
// is returned.
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := VGLutil.Amount(output.Value).ToCoin()
...
func makeTicketSummary(rpc *vgld.RPC, dbtx walletdb.ReadTx, w *Wallet,
...
func (w *Wallet) GetTicketInfoPrecise(ctx context.Context, rpc *vgld.RPC, hash *chainhash.Hash) (*TicketSummary, *wire.BlockHeader, error) {
...
func (w *Wallet) GetTicketsPrecise(ctx context.Context, rpc *vgld.RPC,
...
func (w *Wallet) Accounts(ctx context.Context) (*AccountsResult, error) {
	...
	err := walletdb.View(ctx, w.db, func(dbtx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*VGLutil.Amount)
...
func (w *Wallet) DumpWIFPrivateKey(ctx context.Context, addr stdaddr.Address) (string, error) {
	...
	defer zero()
	wif, err := VGLutil.NewWIF(privKey.Serialize(), w.chainParams.PrivateKeyID,
		VGLec.STEcdsaSecp256k1)
	if err != nil {
		return "", errors.E(op, err)
	}
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(ctx context.Context, wif *VGLutil.WIF) (string, error) {
...
type StakeInfoData struct {
	BlockHeight  int64
	TotalSubsidy VGLutil.Amount
	Sdiff        VGLutil.Amount
...
func (w *Wallet) StakeInfo(ctx context.Context) (*StakeInfoData, error) {
	...
	err := walletdb.View(ctx, w.db, func(dbtx walletdb.ReadTx) error {
		...
		if deployments.VGLP0001.Active(tipHeight, w.chainParams.Net) {
			tipHeader, err := w.txStore.GetBlockHeader(dbtx, &tipHash)
			if err != nil {
				return err
			}
			sdiff, err := w.nextRequiredVGLP0001PoSDifficulty(dbtx, tipHeader, nil)
			if err != nil {
				return err
			}
			res.Sdiff = sdiff
		...
		for it.Next() {
			...
			if it.SpenderHash != (chainhash.Hash{}) {
				...
				case isVote(spender):
					...
					// earned by fees.
					res.TotalSubsidy += VGLutil.Amount(spender.TxIn[0].ValueIn)
...
func (w *Wallet) StakeInfoPrecise(ctx context.Context, rpc *vgld.RPC) (*StakeInfoData, error) {
		...
		for it.Next() {
			...
			if it.SpenderHash != (chainhash.Hash{}) {
				...
				case isVote(spenderTx):
					...
					// customer's subsidy rather than being just the subsidy
					// earned by fees.
					res.TotalSubsidy += VGLutil.Amount(spenderTx.TxIn[0].ValueIn)
...
func (w *Wallet) LockedOutpoints(ctx context.Context, accountName string) ([]vgldtypes.TransactionInput, error) {
	w.lockedOutpointMu.Lock()
	...
	acctLocked := make([]vgldtypes.TransactionInput, 0, len(allLocked))
	err := walletdb.View(ctx, w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		for i := range allLocked {
			...
			acctLocked = append(acctLocked, vgldtypes.TransactionInput{
				Amount: VGLutil.Amount(output.Value).ToCoin(),
				Txid:   op.hash.String(),
				Vout:   op.index,
				Tree:   tree,
			})
...
// returning the total amount of vigil received for a single wallet
// address.
func (w *Wallet) TotalReceivedForAddr(ctx context.Context, addr stdaddr.Address, minConf int32) (VGLutil.Amount, error) {
	const op errors.Op = "wallet.TotalReceivedForAddr"
	var amount VGLutil.Amount
	err := walletdb.View(ctx, w.db, func(dbtx walletdb.ReadTx) error {
		txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)
...
type sigDataSource struct {
	key    func(stdaddr.Address) ([]byte, VGLec.SignatureType, bool, error)
	script func(stdaddr.Address) ([]byte, error)
}

func (s sigDataSource) GetKey(a stdaddr.Address) ([]byte, VGLec.SignatureType, bool, error) {
	return s.key(a)
}
...
func (w *Wallet) CreateVspPayment(ctx context.Context, tx *wire.MsgTx, fee VGLutil.Amount,
	...
	if !txrules.IsDustAmount(VGLutil.Amount(change), txsizes.P2PKHPkScriptSize, feeRate) {
...
func (w *Wallet) SignTransaction(ctx context.Context, tx *wire.MsgTx, hashType txscript.SigHashType, additionalPrevScripts map[wire.OutPoint][]byte,
	additionalKeysByAddress map[string]*VGLutil.WIF, p2shRedeemScriptsByAddress map[string][]byte) ([]SignatureError, error) {
	...
	err := walletdb.View(ctx, w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			source.key = func(addr stdaddr.Address) ([]byte, VGLec.SignatureType, bool, error) {
				if len(additionalKeysByAddress) != 0 {
					...
					return wif.PrivKey(), VGLec.STEcdsaSecp256k1, true, nil
				}
...
func (w *Wallet) CreateSignature(ctx context.Context, tx *wire.MsgTx, idx uint32, addr stdaddr.Address,
	...
	sig, err = sign.RawTxInSignature(tx, int(idx), prevPkScript, hashType,
		privKey.Serialize(), VGLec.STEcdsaSecp256k1)
	if err != nil {
		return nil, nil, errors.E(op, err)
	}
...
// ValidatePreVGLP0005CFilters verifies that all stored cfilters prior to the
// VGLP0005 activation height are the expected ones.
//
// Verification is done by hashing all stored cfilter data and comparing the
// resulting hash to a known, hardcoded hash.
func (w *Wallet) ValidatePreVGLP0005CFilters(ctx context.Context) error {
	const op errors.Op = "wallet.ValidatePreVGLP0005CFilters"

	// Hardcoded activation heights for mainnet and testnet3. Simnet
	// already follows VGLP0005 rules.
	var end *BlockIdentifier
	switch w.chainParams.Net {
	case wire.MainNet:
		end = NewBlockIdentifierFromHeight(validate.VGLP0005ActiveHeightMainNet - 1)
	case wire.TestNet3:
		end = NewBlockIdentifierFromHeight(validate.VGLP0005ActiveHeightTestNet3 - 1)
	default:
		return errors.E(op, "The current network does not have pre-VGLP0005 cfilters")
	}
	...
	err = validate.PreVGLP0005CFilterHash(w.chainParams.Net, &cfsethash)
	if err != nil {
		return errors.E(op, err)
	}
	return nil
