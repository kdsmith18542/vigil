...
// hash algorithm used at Vigil launch.
//
...
// proof of work hash function to blake3, as defined in VGLP0011, is forced
...
// on the difficulty retarget rules defined in VGLP0011.
//
...
func (w *Wallet) checkDifficultyPositional(dbtx walletdb.ReadTx, header *wire.BlockHeader,
	...
	// calculated difficulty using the algorithm defined in VGLP0011 when the
	...
	if !isBlake3PossiblyActive || !isSolvedBlake3(header) {
		...
		// retarget rules for the blake256 hash algorithm used at Vigil launch.
	...
	// algorithm defined in VGLP0011 requires the block prior to the activation
	...
	// rules for the blake256 hash algorithm used at Vigil launch.
	blake256Diff, err := w.calcNextBlake256Diff(dbtx, prevNode, chain, header.Timestamp)
	if err != nil {
		return err
	}
...
// of parameters using the algorithm defined in VGLP0001.
//
...
func (w *Wallet) ancestorHeaderAtHeight(dbtx walletdb.ReadTx, h *wire.BlockHeader, chain []*BlockNode, height int32) (*wire.BlockHeader, error) {
	...
	case height > int32(h.Height), height < 0:
		return nil, nil // vgld's blockNode.Ancestor returns nil for child heights
	}
...
// target node.
//
// Replaces vgld's internal/blockchain func (node *blockNode).IsAncestorOf(target *blockNode).
...
//
// Replaces vgld's internal/blockchain func (node *blockNode) RelativeAncestor(distance int64).
...
// nextRequiredVGLP0001PoSDifficulty calculates the required stake difficulty for
// the block after the passed previous block node based on the algorithm defined
// in VGLP0001.
func (w *Wallet) nextRequiredVGLP0001PoSDifficulty(dbtx walletdb.ReadTx, curHeader *wire.BlockHeader, chain []*BlockNode) (VGLutil.Amount, error) {
	...
	if nextHeight < stakeDiffStartHeight {
		return VGLutil.Amount(w.chainParams.MinimumStakeDiff), nil
	}
	...
	if nextHeight%intervalSize != 0 {
		return VGLutil.Amount(curDiff), nil
	}
	...
	return VGLutil.Amount(sdiff), nil
}

// NextStakeDifficulty returns the ticket price for the next block after the
// current main chain tip block.  This function only succeeds when VGLP0001 is
...
// trusted full node.
func (w *Wallet) NextStakeDifficulty(ctx context.Context) (VGLutil.Amount, error) {
	const op errors.Op = "wallet.NextStakeDifficulty"
	var sdiff VGLutil.Amount
	err := walletdb.View(ctx, w.db, func(dbtx walletdb.ReadTx) error {
		tipHash, tipHeight := w.txStore.MainChainTip(dbtx)
		if !deployments.VGLP0001.Active(tipHeight, w.chainParams.Net) {
			return errors.E(errors.Deployment, "VGLP0001 is not known to be active")
		}
...
// function only succeeds when VGLP0001 is known to be active.
func (w *Wallet) NextStakeDifficultyAfterHeader(ctx context.Context, h *wire.BlockHeader) (VGLutil.Amount, error) {
	const op errors.Op = "wallet.NextStakeDifficultyAfterHeader"
	if !deployments.VGLP0001.Active(int32(h.Height), w.chainParams.Net) {
		return 0, errors.E(op, errors.Deployment, "VGLP0001 is not known to be active")
	}
	var sdiff VGLutil.Amount
	err := walletdb.View(ctx, w.db, func(dbtx walletdb.ReadTx) error {
		var err error
		sdiff, err = w.nextRequiredVGLP0001PoSDifficulty(dbtx, h, nil)
		return err
	})
	if err != nil {
		return 0, errors.E(op, err)
...
func (w *Wallet) validateHeaderChainDifficulties(dbtx walletdb.ReadTx, chain []*BlockNode, idx int) ([]*BlockNode, error) {
	...
	for ; idx < len(chain); idx++ {
		...
		if deployments.VGLP0001.Active(int32(h.Height), w.chainParams.Net) {
			sdiff, err := w.nextRequiredVGLP0001PoSDifficulty(dbtx, parent, chain)
			if err != nil {
				return nil, errors.E(op, err)
			}
			if VGLutil.Amount(h.SBits) != sdiff {
				err := errors.Errorf("%v has invalid PoS difficulty, got %v, want %v",
					hash, VGLutil.Amount(h.SBits), sdiff)
				return chain[idx:], errors.E(op, errors.Consensus, err)
			}
		}
