...
var splitPoints = [...]VGLutil.Amount{
	1 << 36, // 687.19476736
	1 << 34, // 171.79869184
	1 << 32, // 042.94967296
	1 << 30, // 010.73741824
...
func smallestMixChange(feeRate VGLutil.Amount) VGLutil.Amount {
...
func (w *Wallet) MixOutput(ctx context.Context, output *wire.OutPoint, changeAccount, mixAccount,
	...
	err = walletdb.View(ctx, w.db, func(dbtx walletdb.ReadTx) error {
		...
		amount = VGLutil.Amount(txDetails.MsgTx.TxOut[output.Index].Value)
		return nil
	})
	if err != nil {
		w.lockedOutpointMu.Unlock()
	...
	var i int
	var count uint32
	var mixValue, remValue, changeValue VGLutil.Amount
...
SplitPoints:
	for i = 0; i < len(splitPoints); i++ {
		...
		for ; count > 0; count-- {
			remValue = amount - VGLutil.Amount(count)*mixValue
			if remValue < 0 {
				continue
			}
