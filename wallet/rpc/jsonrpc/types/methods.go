...
func NewLockUnspentCmd(unlock bool, transactions []vgldtypes.TransactionInput) *LockUnspentCmd {
...
// the SignRawTransactionCmd struct.  Contains Vigil additions.
...
// ValidatePreVGLP0005CFCmd defines the validatepreVGLP0005cf JSON-RPC command.
type ValidatePreVGLP0005CFCmd struct{}
...
func init() {
	...
	register := []registeredMethod{
		...
		{"validatepreVGLP0005cf", (*ValidatePreVGLP0005CFCmd)(nil)},
	...
	for i := range register {
		VGLjson.MustRegister(Method(register[i].method), register[i].cmd,
			VGLjson.UFWebsocketOnly)
	}
}

// newtype definitions of vgld commands we implement.
type (
	AuthenticateCmd         vgldtypes.AuthenticateCmd
	CreateRawTransactionCmd vgldtypes.CreateRawTransactionCmd
	DebugLevelCmd           vgldtypes.DebugLevelCmd
	GetBestBlockCmd         vgldtypes.GetBestBlockCmd
	GetBestBlockHashCmd     vgldtypes.GetBestBlockHashCmd
	GetBlockCountCmd        vgldtypes.GetBlockCountCmd
	GetBlockHashCmd         vgldtypes.GetBlockHashCmd
	GetBlockHeaderCmd       vgldtypes.GetBlockHeaderCmd
	GetBlockCmd             vgldtypes.GetBlockCmd
	GetCFilterV2Cmd         vgldtypes.GetCFilterV2Cmd
	GetCurrentNetCmd        vgldtypes.GetCurrentNetCmd
	GetInfoCmd              vgldtypes.GetInfoCmd
	GetPeerInfoCmd          vgldtypes.GetPeerInfoCmd
	GetTxOutCmd             vgldtypes.GetTxOutCmd
	HelpCmd                 vgldtypes.HelpCmd
	SendRawTransactionCmd   vgldtypes.SendRawTransactionCmd
	ValidateAddressCmd      vgldtypes.ValidateAddressCmd
