...
the management of the wallet and its connection to the Vigil network.  It has
...
- `string network_address`: The host/IP and optional port of the RPC server to
  ...
  default port is chosen corresponding to the default vgld RPC port of the
  active Vigil network.
...
- `string public_key`: The public key encoded as a string in the Vigil encoding
  format.
...
the Vigil network.  If the transaction spends any of the wallet's unspent
...
following VGLP0005 activation and are therefore safe for use.
...
operations fail if the wallet is not connected to an underlying vgld instance
