...
// Flags.
var opts = struct {
	TestNet               bool    `long:"testnet" description:"Use the test Vigil network"`
	SimNet                bool    `long:"simnet" description:"Use the simulation Vigil network"`
...
func init() {
	...
	if opts.TestNet && opts.SimNet {
		fatalf("Multiple Vigil networks may not be used simultaneously")
	}
	if opts.TestNet {
...
func makeInputSource(outputs []types.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue   VGLutil.Amount
		inputs            = make([]*wire.TxIn, 0, len(outputs))
		redeemScriptSizes = make([]int, 0, len(outputs))
		sourceErr         error
	)
	for _, output := range outputs {
		outputAmount, err := VGLutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(VGLutil.Amount) (*txauthor.InputDetail, error) {
...
func sweep(ctx context.Context) error {
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := VGLutil.Amount(atx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount VGLutil.Amount) bool {
	return amount >= 0 && amount <= VGLutil.MaxAmount
}
