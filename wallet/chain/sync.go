...
// notifications from a vgld JSON-RPC server.
type Syncer struct {
...
// websocket connection to a vgld JSON-RPC server.
...
// NewSyncer creates a Syncer that will sync the wallet using vgld JSON-RPC.
...
// RPC returns the JSON-RPC client to the underlying vgld node.
func (s *Syncer) RPC() *vgld.RPC {
	return s.rpc
}
...
// getMissingHeaders fetches missing headers one by one from vgld. This assumes
// that the initial header sync and rescan have been performed and that vgld's
...
func (s *Syncer) getMissingHeaders(ctx context.Context) error {
	...
	// Perform a rescan of this block on vgld's side, to have the same data
...
func (s *Syncer) waitRPCSync(ctx context.Context, minHeight int64) error {
	...
	for {
		...
		log.Infof("Waiting for vgld instance to catch up to minimum block "+
...
// the vgld server are performed.  Otherwise, it will listen for notifications
...
func (s *Syncer) Run(ctx context.Context) (err error) {
	...
	var api struct {
		Version semver `json:"vgldjsonrpcapi"`
	}
...
func (s *Syncer) winningTickets(ctx context.Context, params json.RawMessage) error {
	block, height, winners, err := vgld.WinningTickets(params)
	if err != nil {
		return err
	}
...
func (s *Syncer) blockConnected(ctx context.Context, params json.RawMessage) error {
	header, relevant, err := vgld.BlockConnected(params)
	if err != nil {
		return err
	}
...
func (s *Syncer) relevantTxAccepted(ctx context.Context, params json.RawMessage) error {
	tx, err := vgld.RelevantTxAccepted(params)
	if err != nil {
		return err
	}
...
func (s *Syncer) mixMessage(ctx context.Context, params json.RawMessage) error {
	...
	msg, err := vgld.MixMessage(params)
	if err != nil {
		return err
	}
